...
import {
  ChevronRight, Search, X, Sparkles, Cpu, Truck, Building2,
} from 'lucide-react';
...
interface CategoryLike { id: string; label: string; color?: string; icon?: LucideIcon; hasSubcategories?: boolean; }
interface SubLike       { id: string; label: string; }
interface ProfLike      { id: string; label: string; }
...
function buildSearchIndex(): Omit<SearchEntry, 'label' | 'catLabel' | 'subLabel'>[] {
  const index: Omit<SearchEntry, 'label' | 'catLabel' | 'subLabel'>[] = [];
  (REGISTRATION_CATEGORIES as CategoryLike[]).forEach(cat => {
    if (cat.hasSubcategories) {
      const subs = getSubcategories(cat.id) as SubLike[];
      subs.forEach(sub => {
        const profs = getProfessions(cat.id, sub.id) as ProfLike[];
        profs.forEach(prof => {
          index.push({ prof, cat, sub, rawKey: `${prof.label} ${prof.id} ${cat.label} ${sub.label}`.toLowerCase() });
        });
      });
    } else {
      const profs = getProfessions(cat.id, null) as ProfLike[];
      profs.forEach(prof => {
        index.push({ prof, cat, sub: null, rawKey: `${prof.label} ${prof.id} ${cat.label}`.toLowerCase() });
      });
    }
  });
  return index;
...
let _cachedRawIndex: Omit<SearchEntry, 'label' | 'catLabel' | 'subLabel'>[] | null = null;
function getRawIndex() {
  if (!_cachedRawIndex) _cachedRawIndex = buildSearchIndex();
  return _cachedRawIndex;
}

interface SearchResultRowProps { item: SearchEntry; onDirectSelect: (cat: CategoryLike, sub: SubLike | null, prof: ProfLike) => void; }
function SearchResultRow({ item, onDirectSelect }: SearchResultRowProps) {
  const BadgeIcon = (item.cat.icon as LucideIcon | undefined) ?? Sparkles;
  return (
    <motion.button type="button" onClick={() => onDirectSelect(item.cat, item.sub, item.prof)}
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-xs text-white/50">{item.catLabel}{item.subLabel ? ` › ${item.subLabel}` : ''}</p>
      </div>
...
interface CategoryCardProps { cat: CategoryLike; t: (key: string, opts?: object) => string; onSelect: (cat: CategoryLike) => void; }
function CategoryCard({ cat, t, onSelect }: CategoryCardProps) {
  const ring = COLOR_RING[cat.color ?? 'slate'] ?? COLOR_RING['slate']!;
  const Icon = (cat.icon as LucideIcon | undefined) ?? Sparkles;
  return (
    <motion.button type="button" onClick={() => onSelect(cat)}
      whileHover={{ y: -6, scale: 1.02, rotateX: 2 }} whileTap={{ scale: 0.985 }}
      ...
      style={{ boxShadow: '0 16px 40px rgba(0,0,0,.18)', transformStyle: 'preserve-3d' }}>
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        style={{ background: 'radial-gradient(circle at 30% 20%, rgba(255,255,255,.08), transparent 35%)' }} />
...
interface RecommendedSectionProps {
  ...
  onDirectSelect: (cat: CategoryLike, sub: SubLike | null, prof: ProfLike) => void;
  onSelect: (cat: CategoryLike) => void;
}

function RecommendedSection({ t, onSelect }: RecommendedSectionProps) {
  const recommendations = [
    { id: 'construction', label: 'Construction', icon: Building2, note: 'Buildings, repairs, and site work.' },
    { id: 'technology',   label: 'Technology',   icon: Cpu,       note: 'Software, IT, and digital work.'  },
    { id: 'delivery',     label: 'Delivery',     icon: Truck,     note: 'Transport, courier, and logistics.'},
  ];
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <p className="text-sm leading-6 text-white/70">Based on your profile, we recommend these starting categories:</p>
        ...
        {recommendations.map(item => {
          ...
          return (
            <button key={item.id} type="button"
              onClick={() => {
                const cat = (REGISTRATION_CATEGORIES as CategoryLike[]).find(c => c.id === item.id);
                if (cat) onSelect(cat);
              }}
...
export default function Step1_CategorySelect({ onSelect, onDirectSelect }: Step1CategorySelectProps) {
  const { t } = useTranslation();
  const [query, setQuery] = useState('');

  const translatedIndex = useMemo<SearchEntry[]>(() =>
    getRawIndex().map(item => ({
      ...item,
  ...
  const handleDirectSelect = (cat: CategoryLike, sub: SubLike | null, prof: ProfLike) => {
    onDirectSelect?.(cat, sub, prof);
  };
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-[32px] px-4 py-6 md:px-6 md:py-8" style={{ background: BG, color: '#fff' }}>
      <motion.div className="pointer-events-none absolute inset-0"
        animate={{ scale: [1, 1.06, 1] }} transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }}
        style={{ background: 'radial-gradient(circle at top, #1e293b, transparent 42%)' }} />

      <div className="relative mx-auto w-full max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: 'easeOut' }}
          className="mb-4 rounded-[28px] border border-white/10 bg-white/5 p-5 backdrop-blur-2xl">
          <p className="text-[10px] font-bold uppercase tracking-[0.34em] text-white/45">Step 2 of 6</p>
          <h2 className="mt-2 text-[22px] font-black tracking-tight text-white md:text-[26px]">Find the right profession faster</h2>
          <p className="mt-2 text-sm leading-6 text-white/70 md:text-[15px]">Search by profession, or browse categories to continue.</p>
          <div className="mt-3 text-[11px] font-medium text-white/45">33%</div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.06, ease: 'easeOut' }} className="relative mb-4">
        ...
        {showResults && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: 'easeOut' }} className="mb-4">
            {results.length === 0 ? (
              ...
              <div className="space-y-2">
                <div className="px-1 pb-1 text-[11px] font-bold uppercase tracking-[0.28em] text-white/40">{t('registration.ui.searchResults')}</div>
                {results.map((item, idx) => (
                  <SearchResultRow key={`${item.cat.id}-${item.sub?.id ?? 'null'}-${item.prof.id}-${idx}`}
                    item={item} onDirectSelect={handleDirectSelect} />
                ))}
              </div>
            )}
          </motion.div>
        ...
        {!showResults && (
          <>
            <RecommendedSection t={t} onSelect={onSelect} onDirectSelect={handleDirectSelect} />
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.08, ease: 'easeOut' }}>
              <h2 className="mb-3 text-[11px] font-bold uppercase tracking-[0.28em] text-white/40">{t('registration.ui.selectCategory')}</h2>
