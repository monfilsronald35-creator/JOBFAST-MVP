...
const ACCOUNT_TYPES: AccountType[] = [
  {
    id: 'personal', label: 'For Individuals', headline: 'Find Jobs',
    desc1: 'Offer Services', desc2: 'Build Your Career',
    ...
    bgIdle: 'rgba(20,30,52,.56)', bgHover: 'rgba(20,30,52,.72)', icon: UserRound,
  },
  {
    id: 'business', label: 'For Organizations', headline: 'Hire Talent',
    desc1: 'Recruit Teams', desc2: 'Manage Your Company',
    ...
    borderIdle: 'rgba(255,255,255,0.08)', borderHover: 'rgba(96,165,250,0.42)',
    bgIdle: 'rgba(20,30,52,.56)', bgHover: 'rgba(20,30,52,.72)', icon: Building2,
  },
];
  ...
  return (
    <section className="relative h-[236px] overflow-hidden rounded-[32px] border border-white/10 bg-white/5 shadow-[0_24px_80px_rgba(0,0,0,.42)] backdrop-blur-[28px] md:h-[244px]">
      <div className="absolute inset-0">
        <motion.img src="/assets/onboarding/account-type-hero.webp" alt="JOBFAST hero"
          className="h-full w-full object-cover opacity-95" loading="eager" decoding="async"
      ...
      <div className="relative flex h-full flex-col items-center justify-center px-5 text-center md:px-8">
        <p className="text-[10px] font-bold uppercase tracking-[0.34em] text-white/50 md:text-[11px]">WELCOME TO JOBFAST</p>
        <h1 className="mt-2 text-[24px] font-black tracking-tight text-white md:text-[28px]">Choose your account type.</h1>
...
function RoleCard({ item, active, inactive, onSelect }: RoleCardProps) {
  ...
  return (
    <motion.button type="button" onClick={() => onSelect(item.id)}
      whileHover={{ y: -6, scale: 1.02 }} whileTap={{ scale: 0.985 }}
      ...
      <div className={`relative grid gap-4 p-4 md:grid-cols-[200px_1fr_auto] md:items-center md:p-5 ${active ? 'translate-y-[-3px]' : ''}`}>
        <div className="relative overflow-hidden rounded-[24px] border border-white/10 bg-black/20">
          <motion.img src={item.preview} alt={item.label} className="h-[155px] w-full object-cover md:h-[170px]"
            loading="lazy" decoding="async" whileHover={{ scale: 1.06, rotate: 0.4 }} transition={{ duration: 0.35, ease: 'easeOut' }} />
        ...
        <div className="min-w-0">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {item.badges.map(badge => (
              <span key={badge} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-medium text-white/75">
                ✓ {badge}
              </span>
            ))}
          </div>
...
interface Step0AccountTypeProps { onSelect?: (id: string) => void; }

export default function Step0_AccountType({ onSelect }: Step0AccountTypeProps) {
  const [selected, setSelected] = useState<string | null>(null);
  const active = useMemo(() => ACCOUNT_TYPES.find(x => x.id === selected) ?? null, [selected]);

  const continueLabel =
    selected === 'personal' ? 'Continue as Individual' :
    selected === 'business' ? 'Continue as Organization' :
    'Continue';
      ...
      <div className="mx-auto w-full max-w-3xl space-y-5">
        ...
        <div className="space-y-4">
          {ACCOUNT_TYPES.map(type => (
            <RoleCard key={type.id} item={type}
        ...
        <p className="pb-2 text-center text-xs text-white/45">Ou ka toujou chanje kalite kont ou pita.</p>
      </div>
    </div>
  );
}