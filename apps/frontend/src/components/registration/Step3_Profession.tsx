...
interface ProfessionItem { id: string; label: string; [key: string]: unknown; }
interface CategoryLike   { id: string; label: string; icon?: string; }
interface SubcategoryLike { id: string; label: string; }

interface Step3ProfessionProps {
  category:     CategoryLike;
  subcategory?: SubcategoryLike | null;
  onSelect:     (profession: ProfessionItem) => void;
  onBack:       () => void;
}

export default function Step3_Profession({ category, subcategory, onSelect, onBack }: Step3ProfessionProps) {
  const { t } = useTranslation();
  const [selected, setSelected] = useState<ProfessionItem | null>(null);

  const professions = getProfessions(category.id, subcategory?.id ?? null) as ProfessionItem[];
        ...
        {professions.map((p) => {
          ...
          return (
            <button key={p.id} type="button" onClick={() => setSelected(p)}
              ...
              {selected?.id === p.id && <span className="text-indigo-400 text-xs font-bold">✓</span>}
            </button>
          );
        })}
      </div>
