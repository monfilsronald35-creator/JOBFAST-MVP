...
function AvatarUpload({ onPhotoChange, initialPhoto, userName = 'User' }: AvatarUploadProps) {
  const [preview, setPreview] = useState<string | undefined>(initialPhoto);
  const [loading, setLoading] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const getInitialLetter = () => userName.charAt(0).toUpperCase();
  ...
  if (preview) {
    return (
      <div className="relative w-32 h-32 mx-auto mb-4">
        <img src={preview} alt="Profile" className="w-full h-full rounded-full object-cover border-4 border-yellow-400" />
        ...
        <input ref={fileInputRef} type="file" accept="image/*" onChange={handlePhotoSelect} className="hidden" disabled={loading} />
      </div>
    );
  }
