...
function mockFeed(user: UserLike | null | undefined): FeedEntry[] {
  return [
...
function Avatar({ name = '', avatar, size = 14 }: { name?: string; avatar?: string | null; size?: number }) {
  const letter = (name[0] ?? '?').toUpperCase();
  const idx    = name.charCodeAt(0) % RING_COLORS.length;
  const ringClass = RING_COLORS[idx] ?? RING_COLORS[0]!;
  return (
    <div className={`w-${size} h-${size} rounded-full bg-gradient-to-br ${ringClass} flex items-center justify-center text-white font-black text-xs shadow-lg shrink-0 overflow-hidden`}>
      {avatar ? <img src={avatar} alt={name} className="w-full h-full object-cover" /> : letter}
    </div>
  );
}

interface StoryViewerProps { feed: FeedEntry[]; startIndex: number; onClose: () => void; }

function StoryViewer({ feed, startIndex, onClose }: StoryViewerProps) {
  ...
  }, [userIdx, storyIdx, paused, goNext, story]);
      ...
      <div className="flex-1 flex items-center justify-center relative">
        ...
        {story.type === 'text' && (
          <div className="px-8 text-center">
            <p className="text-2xl font-black text-white drop-shadow-lg leading-relaxed">{story.text}</p>
          </div>
        )}
...
interface CreateStorySheetProps { onClose: () => void; onCreated: (story: StoryItem) => void; }

function CreateStorySheet({ onClose, onCreated }: CreateStorySheetProps) {
  const { t }    = useTranslation();
  const { user } = useAuth() as { user: UserLike | null };
  const [type,    setType]    = useState<'text' | 'image' | 'video'>('text');
  const [text,    setText]    = useState('');
  const [bg,      setBg]      = useState(TEXT_BG_OPTS[0] ?? 'from-pink-600 to-rose-500');
  ...
  const handlePost = async () => {
    ...
    setPosting(true);
    try {
      const payload = { id: String(Date.now()), type, bg, text, userId: user?._id, postedAt: Date.now() };
      const res = await (storiesAPI.create(payload) as Promise<{ data?: { data?: StoryItem } }>);
      onCreated((res?.data?.data as StoryItem | undefined) ?? payload);
    } catch {
      onCreated({ id: String(Date.now()), type, bg, text, postedAt: Date.now() });
    }
    setPosting(false);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end">
      ...
      <div className="relative w-full bg-[#0d1526] rounded-t-3xl z-10 overflow-hidden max-h-[85vh] overflow-y-auto">
        ...
        <h3 className="text-base font-bold px-5 mb-4 text-white">{t('stories.create', { defaultValue: 'Kreye Istwa' })}</h3>

        <div className="flex gap-2 px-5 mb-4">
          {([
            { id:'text',  icon:'✍️', label:'Tèks'  } as const,
            { id:'image', icon:'📷', label:'Photo'  } as const,
            { id:'video', icon:'🎥', label:'Videyo' } as const,
          ]).map(opt => (
...
export default function StoryRing() {
  ...
  useEffect(() => {
    (storiesAPI.getFeed() as Promise<{ data?: { data?: FeedEntry[] } | FeedEntry[] }>)
      .then(res => {
        const raw = res?.data;
        const data: FeedEntry[] | undefined = Array.isArray(raw)
          ? raw
          : (raw as { data?: FeedEntry[] } | undefined)?.data;
  ...
  const handleCreated = (newStory: StoryItem) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-[10px] font-semibold text-slate-400 max-w-[56px] truncate text-center">
                {isOwn ? t('stories.myStory', { defaultValue: 'Istwa Mwen' }) : (person.name.split(' ')[0] ?? person.name)}
              </p>
            </button>
          );
        })}
