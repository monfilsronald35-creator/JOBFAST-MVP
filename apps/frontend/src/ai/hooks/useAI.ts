...
export function useAI(options: UseAIOptions = {}) {
  ...
  const send = useCallback(async (content: string): Promise<string> => {
    ...
    try {
      ...
      const response = await AIGateway.chat({
        ...
        context:      options.userId ? { userId: options.userId, sessionId: sessionId.current } : undefined,
      });
      ...
      if (options.userId) {
        const ts = Date.now();
        await AIMemory.addMessage({ role: 'user',      content,          sessionId: sessionId.current, userId: options.userId, timestamp: ts });
        await AIMemory.addMessage({ role: 'assistant', content: assistantContent, sessionId: sessionId.current, userId: options.userId, timestamp: ts + 1 });
      }
  ...
  const stream = useCallback(async (content: string): Promise<void> => {
    setError(null);
    setLoading(true);
    setStreaming('');
