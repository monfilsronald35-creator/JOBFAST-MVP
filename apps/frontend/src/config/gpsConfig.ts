export const GPS_RADIUS_OPTIONS: readonly number[] = [2, 5, 10, 25, 50, 100];
export const GPS_DEFAULT_RADIUS_KM = 10;

export const GPS_CACHE_TTL_MS           = 5  * 60 * 1000;
export const GPS_THROTTLE_MS            = 30 * 1000;
export const GPS_BACKGROUND_THROTTLE_MS = 2  * 60 * 1000;
...
export type GpsState = (typeof GPS_STATES)[keyof typeof GPS_STATES];

export const GPS_STATE_LABELS: Record<GpsState, string> = {
  idle:         'Atann',
...
export const CLUSTER_GRID_KM = 1.5;
...
export const OFFLINE_CITIES: readonly OfflineCity[] = [
...
export const ROLE_AVAILABILITY_STATES: Record<string, AvailabilityStateEntry[]> = {
  worker: [
...
export function getRoleAvailabilityStates(role: string): AvailabilityStateEntry[] {
  return ROLE_AVAILABILITY_STATES[role] ?? (ROLE_AVAILABILITY_STATES['user'] ?? []);
}

export const NEARBY_SEARCHABLE_ROLES: readonly string[] = [
  'worker', 'company', 'enterprise',
  'restaurant', 'hotel', 'rental', 'office',
  'tourism', 'hospital', 'clinic', 'service_provider',
];

export const NEARBY_DEFAULT_ROLES: readonly string[] = [
  'worker', 'restaurant', 'hotel', 'service_provider',
];

export const ROLE_NEARBY_ICONS: Record<string, string> = {
  worker:           '👷',
  company:          '🏢',
  enterprise:       '🏛️',
  restaurant:       '🍽️',
...
export function getRoleNearbyIcon(role: string): string {
