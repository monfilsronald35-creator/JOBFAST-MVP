...
type SubcategoryProfessions = Record<string, readonly ProfessionEntry[]>;
type FlatProfessions        = readonly ProfessionEntry[];

type ProfessionMap = Record<string, SubcategoryProfessions | FlatProfessions>;

export const PROFESSIONS: ProfessionMap = {
...
export function getSubcategories(categoryId: string): readonly Subcategory[] {
  return SUBCATEGORIES[categoryId] ?? [];
}

export function getProfessions(categoryId: string, subcategoryId?: string | null): readonly ProfessionEntry[] {
  const cat = PROFESSIONS[categoryId];
  if (!cat) return [];
  if (Array.isArray(cat)) return cat as FlatProfessions;
  const sub = cat as SubcategoryProfessions;
  if (subcategoryId && sub[subcategoryId]) return sub[subcategoryId] ?? [];
  return [];
}