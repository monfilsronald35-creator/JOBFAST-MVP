...
const STATUS_OPTIONS = [
  ...
  },
  {
    ...
    badgeClass: "bg-gold-400/10 text-gold-400 border-gold-400/20",
    icon: (
  ...
  },
  {
    ...
    badgeClass: "bg-danger-500/10 text-danger-500 border-danger-500/20",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  },
  {
    ...
    badgeClass: "bg-slate-500/10 text-text-muted border-slate-800",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <div className="min-h-screen w-full bg-navy-900 text-text-inverse flex flex-col font-sans select-none justify-between pb-8">
      ...
      <div className="px-5 pt-6 pb-4 flex items-center justify-between border-b border-navy-800/60 z-10">
        <button 
          onClick={() => navigate(-1)} 
          className="p-2 bg-navy-800 rounded-xl text-text-muted hover:text-gold-400 active:scale-95 transition-all"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-sm font-extrabold tracking-wider uppercase text-slate-200">Estati Disponibilite</h1>
              ...
              <button
                ...
                className={`p-4 rounded-2xl border text-left flex items-start gap-4 transition-all duration-200 relative ${
                  isSelected 
                    ...
                    : "bg-navy-800/40 border-navy-800 hover:border-slate-800/80"
                }`}
              >
                {/* Endikatè Koulè / Ikòn */}
                ...
                <div className="absolute top-4 right-4 flex items-center justify-center">
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${
                    isSelected ? "border-gold-400" : "border-slate-800"
                  }`}>
                    {isSelected && <div className="w-1.5 h-1.5 bg-gold-400 rounded-full"></div>}
                  </div>
                </div>
              </button>
            );
      ...
      {/* 🧭 BOUTON ANBA (ACTION CONFIRMATION) */}
