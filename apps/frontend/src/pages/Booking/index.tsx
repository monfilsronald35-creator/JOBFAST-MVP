...
interface CreateBookingModalProps {
  onClose:   () => void;
  onCreated: (b: BookingRecord) => void;
}
...
const STATUS_STYLE: Record<string, StatusEntry> = {
...
const FALLBACK_STATUS: StatusEntry = STATUS_STYLE["pending"]!;
...
function Field({ label, type = "text", value, onChange, placeholder, options }: FieldProps) {
  return (
    <div>
...
function CreateBookingModal({ onClose, onCreated }: CreateBookingModalProps) {
  const { t }    = useTranslation();
  const { user } = useAuth();
  const userRecord = user as Record<string, unknown> | null;

  const [step,        setStep]        = useState(1);
  const [bookingType, setBookingType] = useState<BookingTypeConfig | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const userId = (userRecord?.["_id"] as string | undefined) ?? "";
    try {
      const res = await (bookingAPI.create as (p: Record<string, unknown>) => Promise<{ data?: { data?: BookingRecord } }>)(
        { ...form, type: bookingType.id, userId },
      );
      const created: BookingRecord = res?.data?.data ?? {
        _id: String(Date.now()),
        ...form,
        type: bookingType.id,
        status: "pending",
  ...
  const colorCls = bookingType ? COLOR_BG[bookingType.color] ?? "" : "";
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800">
        {step > 1 ? (
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-800 text-slate-300"
          >
            ←
          </button>
        ) : (
        ...
        <h2 className="font-bold text-white flex-1">
          {t("booking.newBooking", { defaultValue: "Nouvo Rezèvasyon" })}
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-sm text-slate-400 mb-4">
              {t("booking.chooseType", { defaultValue: "Pou ki sèvis?" })}
            </p>
              ...
              {BOOKING_TYPES.map((bt) => (
                ...
                >
                  ...
                  <span className="text-sm font-bold capitalize">
                    {t(`booking.type.${bt.id}`, { defaultValue: bt.id })}
                  </span>
                </button>
              ))}
            </div>
        ...
        {step === 2 && bookingType && (
          <div className="space-y-4">
            <div className={`flex items-center gap-3 p-3 rounded-xl border ${colorCls}`}>
              <span className="text-2xl">{bookingType.icon}</span>
              <span className="font-bold capitalize">
                {t(`booking.type.${bookingType.id}`, { defaultValue: bookingType.id })}
              </span>
            </div>

            <Field
              label={t("booking.provider", { defaultValue: "Non sèvis la" })}
              value={form["provider"] ?? ""}
              onChange={(v) => set("provider", v)}
              placeholder="Ex: Hôtel Oloffson"
            />
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-indigo-900/30 border border-indigo-700/40 rounded-2xl">
              <p className="text-xs font-bold text-indigo-300">
                🔒 {t("booking.escrowProtect", { defaultValue: "Peman JOBFAST Escrow" })}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {t("booking.escrowNote", { defaultValue: "Peman an kenbe an sekurité jiskaske rezèvasyon an konfime." })}
              </p>
            </div>

            <div className="space-y-2">
              <p className="text-xs text-slate-400 uppercase tracking-wide">
                {t("booking.payWith", { defaultValue: "Peye Avèk" })}
              </p>
      ...
      <div className="p-4 border-t border-slate-800 bg-[#020617]">
        {step < 3 ? (
          ...
          <button
            ...
            disabled={submitting || !form["payMethod"]}
...
function BookingCard({ booking, onCancel }: BookingCardProps) {
  ...
  const st = STATUS_STYLE[booking.status] ?? FALLBACK_STATUS;

  return (
    <div className="bg-[#0d1526] border border-slate-800 rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        <div
          className={`w-11 h-11 rounded-2xl flex items-center justify-center text-xl border ${COLOR_BG[bt.color] ?? ""} shrink-0`}
        >
          {bt.icon}
        </div>
        ...
        <div className="text-right shrink-0">
          <div className={`flex items-center gap-1.5 ${st.text}`}>
            ...
            <span className="text-xs font-bold">
              {t(`booking.status.${booking.status}`, { defaultValue: st.label })}
            </span>
          </div>
          {booking.amount > 0 && (
      ...
      {booking.guests && (
        <div className="px-4 pb-3 flex items-center gap-2 text-xs text-slate-500">
          ...
          {booking.doctor && (
            <><span>·</span><span>👨‍⚕️ {booking.doctor}</span></>
          )}
        </div>
      )}
...
export default function BookingPage() {
  ...
  useEffect(() => {
    setLoading(true);
    (bookingAPI.getMyBookings as () => Promise<{ data?: { data?: BookingRecord[] } | BookingRecord[] }>)()
      .then((res) => {
        const raw = res?.data;
        const d = Array.isArray(raw)
          ? raw
          : (raw as { data?: BookingRecord[] } | undefined)?.data;
  ...
  const handleCancel = useCallback(async (id: string) => {
    try {
      await (bookingAPI.cancel as (id: string, reason: string) => Promise<void>)(id, "user_request");
    } catch {}
      ...
      <div className="sticky top-14 z-30 bg-[#020617]/97 backdrop-blur-xl border-b border-slate-800/60 px-4 pt-3 pb-3">
        <div className="flex items-center justify-between mb-3">
          <h1 className="text-base font-black">
            📅 {t("booking.title", { defaultValue: "Rezèvasyon Mwen" })}
          </h1>
          <button
            type="button"
        ...
        {!loading && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-slate-400 text-sm">
              {t("booking.empty", { defaultValue: "Pa gen rezèvasyon" })}
            </p>
            <button
              type="button"
              onClick={() => setShowCreate(true)}
