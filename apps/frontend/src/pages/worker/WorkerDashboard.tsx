...
export function computeCompleteness(user: UserRecord): CompletenessResult {
  ...
  const checks = [
    { done: !!wu?.name,                                   tip: 'Ajoute non konplè ou' },
    { done: !!wu?.email,                                  tip: 'Imèl verifye' },
    { done: !!(meta.phone ?? wu?.phone),                  tip: 'Ajoute nimewo telefòn ou' },
    { done: !!meta.bio,                                   tip: 'Ekri yon bio pou pwofil ou' },
    { done: !!wu?.profession,                             tip: 'Chwazi yon metye' },
    { done: !!(wu?.location?.city ?? meta.city),          tip: 'Ajoute lokasyon ou' },
    { done: (meta.skills?.length ?? 0) > 0,               tip: 'Ajoute konpetans ou yo' },
    { done: (meta.workPhotos?.length ?? 0) > 0,           tip: 'Ajoute foto travay ou' },
    { done: !!(meta.yearsExperience ?? wu?.experience),   tip: 'Espesifye eksperyans ou' },
  ];
...
const HeroCard = memo(function HeroCard({ user }: { user: UserRecord }) {
  const wu         = toW(user);
  const name       = wu?.name ?? wu?.firstName ?? 'Travayè';
    ...
    >
      ...
      <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col gap-2 min-w-0">
          <p className="text-[11px] text-slate-400 uppercase tracking-[0.18em]">Good Morning {name}</p>
          <h2 className="text-lg sm:text-xl font-semibold text-white leading-tight">Dashboard Travayè JOBFAST</h2>
    ...
    <div className="space-y-4">
      ...
      <Section>
        <div className="grid grid-cols-3 gap-3 mb-3">
          <StatCard value={`⭐ ${(wu?.stats?.rating ?? 5).toFixed(1)}`} label="Rating" />
          <StatCard value={wu?.stats?.totalJobs ?? 0}                  label="Travay Fini" color="green" />
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex flex-wrap gap-2 mb-3 min-h-[24px]">
          {skills.length === 0 && <p className="text-xs text-slate-500">Pa gen konpetans ajoute</p>}
          {skills.map(skill => (
      ...
      <Section icon="🎓" title="Eksperyans Pwofesyonèl">
        <dl className="space-y-2.5">
          {([
            ['Ane Eksperyans', `${meta.yearsExperience ?? wu?.experience ?? 0} an`],
            ['Metye',          wu?.profession   ?? '—'],
            ['Kategori',       wu?.category     ?? '—'],
            ['Kont',           wu?.accountType  ?? '—'],
            ['Manm depi',      wu?.stats?.memberSince ?? '—'],
      ...
      <Section icon="✅" title="Verifikasyon">
        <div className="space-y-2.5">
          {[
            { label: 'Imèl',     done: !!wu?.email,                       icon: '📧' },
            { label: 'Telefòn',  done: !!(wu?.phone ?? meta.phone),        icon: '📱' },
...
function ScheduleTab({ user, jobs }: { user: UserRecord; jobs: JobItem[] }) {
  ...
  }, [acquire, user, wu, userId, login, serviceRadius]);
    ...
    <div className="space-y-4">
      ...
      <Section icon="⭐" title="Evalyasyon">
        <div className="flex flex-col items-center py-4">
          ...
          <p className="text-xs text-slate-400">{totalJobs} evalyasyon • 0 reklamasyon</p>
          {totalJobs === 0 && (
...
function TrustTab({ user }: { user: UserRecord }) {
  ...
  const scoreLabel = trustScore >= 80 ? 'Ekselan — Ou pami meyè yo!' : trustScore >= 50 ? 'Bon — Kontinye amelyore' : 'Kòmanse bati konfyans ou';
