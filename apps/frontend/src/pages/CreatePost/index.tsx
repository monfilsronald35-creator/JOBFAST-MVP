...
import { X, Video, Image, ChevronRight } from "lucide-react";
...
const POST_TYPES: PostType[] = [
  { id: "photo",     icon: "📷", label: "Photo",      sublabel: "Pataje foto travay ou",  accent: "#10b981" },
  { id: "video",     icon: "🎥", label: "Vidéo",      sublabel: "30 – 60 segonn",         accent: "#6366f1" },
  { id: "promotion", icon: "📢", label: "Pwomosyon",  sublabel: "Reklame sèvis ou",       accent: GOLD      },
];
...
export default function CreatePostScreen() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>, mediaType: string) {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => {
      if (ev.target?.result) {
        setMedia({ url: ev.target.result as string, type: mediaType, file });
      }
    };
  ...
  async function handlePost() {
    setPosting(true);
    const myId     = String(userRecord?.['_id'] ?? userRecord?.['id'] ?? '');
    const mediaUrl = media?.url ?? '';
    const profileMeta = userRecord?.['profileMetadata'] as { profilePhoto?: string } | undefined;
    const postData = {
      id:           `post_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
      userId:       myId,
      userName:     (userRecord?.['name'] as string) ?? '',
      userAvatar:   profileMeta?.profilePhoto ?? '',
      type:         postType,
      mediaUrl,
