...
interface FormData { identifier: string; password: string; remember: boolean; }

interface LoginResponse {
  success?: boolean; status?: number; message?: string; token?: string;
...
function LoginPage() {
  ...
  const retryWithBackoff = useCallback(async (credentials: { identifier: string; password: string }, attempt: number) => {
    ...
    try {
      const res = await performLoginApi(credentials) as LoginResponse;
      if (res?.success) {
        ...
        if (mounted.current) {
          setSlowLoad(false); setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await performLoginApi({ identifier, password }) as LoginResponse;
      ...
      setLoading(false); setSlowLoad(false);
      setError(e?.response?.data?.message ?? e?.message ?? t('auth.invalidCredentials'));
    }
  };

  const handleFormKeyDown = (_e: React.KeyboardEvent<HTMLFormElement>) => {
    // form submit handles Enter key
  };
  ...
  const passwordHints =
    passwordScore <= 1 ? t('auth.passwordWeak')   ?? 'Mot de pase a twò fèb.'  :
    passwordScore === 2 ? t('auth.passwordMedium') ?? 'Mot de pase a mwayen.'   :
    ...
    <div className="min-h-screen flex flex-col justify-center bg-navy-900 px-6 relative">
      {offline.isOffline && (
        <div className="absolute top-0 inset-x-0 z-20 bg-amber-500/10 border-b border-amber-500/30 text-amber-300 text-xs text-center py-2 px-4">
          {t('errors.offline') ?? 'Ou offline kounye a. N ap eseye rekonekte otomatikman.'}
        </div>
      )}
      ...
      {loading && slowLoad && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#050B18]/90 backdrop-blur-sm px-8 text-center gap-4">
          ...
          <p className="text-xs text-slate-400 max-w-xs">
            {t('auth.serverSleepHint') ?? 'Sèvè gratuit la dòmi apre kèk min inaktivite. Tann kèk segond — li pral bon!'}
          </p>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-slate-400 mt-2">{t('auth.loginToAccount')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown}
        ...
        {(capsLockOn || formData.password) && (
          <div className="flex justify-between items-center">
            ...
            {formData.password && (
              <span className="ml-auto text-[10px] text-slate-400">{passwordHints}</span>
            )}
          </div>
        )}
