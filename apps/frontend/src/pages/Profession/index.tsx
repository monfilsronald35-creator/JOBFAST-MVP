...
interface ProfessionData {
  ...
  skills: string[];
  certificates: string[];
  countriesHiring: CountryHiring[];
  openJobs: OpenJob[];
  companiesHiring: CompanyHiring[];
  services: ProfService[];
  training: ProfTraining[];
  workerCount: number;
  satisfactionPct: number;
}

const MOCK_PROFESSIONS: Record<string, ProfessionData> = {
  electrician: {
    id: 'electrician', icon: '⚡',
...
const DEFAULT_PROF: ProfessionData = {
  id: 'unknown', icon: '💼',
...
export default function ProfessionDetailPage() {
  ...
  const roleType: string =
    (userRecord?.['roleType'] as string) ??
    (userRecord?.['role'] === 'worker'           ? 'worker'           :
     userRecord?.['role'] === 'service_provider' ? 'service_provider' : 'employer');

  useEffect(() => {
    setLoading(true);
    API.get<unknown>(`/professions/${professionId}`)
      .then(res => {
        const d = res?.data as { data?: ProfessionData } | ProfessionData | null;
        setData((d as { data?: ProfessionData })?.data ?? (d as ProfessionData) ?? null);
      })
      .catch(() => {})
      .finally(() => setLoading(false));

    const mock = professionId
      ? MOCK_PROFESSIONS[professionId] ?? { ...DEFAULT_PROF, id: professionId }
      : { ...DEFAULT_PROF };
  ...
  const primaryAction = () => {
    if (roleType === 'worker')
      navigate('/search', { state: { profession: professionId } });
    else if (roleType === 'service_provider')
      navigate('/provider-dashboard', { state: { tab:'create-service', profession: professionId } });
    else
      navigate('/post-job', { state: { profession: professionId } });
  };
            ...
            {prof.openJobs.length === 0 ? (
              ...
              <div key={job.id} className="p-4 bg-[#0d1526] border border-slate-700 rounded-2xl">
                <div className="flex items-start justify-between mb-2">
                  ...
                  {job.urgent && (
                    <span className="text-[10px] bg-red-500/20 text-red-400 border border-red-500/40 px-2 py-0.5 rounded-full font-bold">🔥 Ijan</span>
                  )}
                </div>
