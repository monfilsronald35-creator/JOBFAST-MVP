...
const AnalyticsTab = memo(function AnalyticsTab({ user: _user }: TabProps) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 space-y-4">
        ...
        <div className="flex items-end gap-2 h-28">
          {MOCK_SERIES.map(({ month, revenue }) => (
            <div key={month} className="flex flex-col items-center gap-1 flex-1">
              <div className="w-full rounded-t-lg bg-gradient-to-t from-amber-500 to-amber-300 transition-all"
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'Total Revenue', value: '$2.3M' }, { label: 'Active Users', value: '12,840' },
          { label: 'Conversion Rate', value: '8.4%' }, { label: 'Avg Order Value', value: '$184' },
        ].map(({ label, value }) => (
