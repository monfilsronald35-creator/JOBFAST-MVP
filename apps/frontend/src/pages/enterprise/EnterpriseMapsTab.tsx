...
const EnterpriseMapsTab = memo(function EnterpriseMapsTab({ user: _user }: TabProps) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 space-y-4">
      <h2 className="text-base font-black text-white">Enterprise Maps</h2>
      <div id="enterprise-map-container"
      ...
      <div className="grid grid-cols-3 gap-3">
        {['Live Tracking', 'Heatmap', 'Zone Analysis'].map((label) => (
          <button key={label} type="button"
            className="rounded-2xl border border-white/10 bg-black/30 px-3 py-2 text-[10px] text-slate-200 hover:border-amber-400/40">
            {label}
          </button>
        ))}
      </div>
