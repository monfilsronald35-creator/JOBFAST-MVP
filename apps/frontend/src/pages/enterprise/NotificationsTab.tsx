...
const NotificationsTab = memo(function NotificationsTab({ user: _user }: TabProps) {
  return (
      ...
      <ul className="space-y-2">
        {CHANNELS.map(({ label, enabled }) => (
          <li key={label} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 px-3 py-2">
            ...
            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${enabled ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-slate-700 bg-slate-800 text-slate-400'}`}>
              {enabled ? 'ON' : 'OFF'}
            </span>
          </li>
        ))}
