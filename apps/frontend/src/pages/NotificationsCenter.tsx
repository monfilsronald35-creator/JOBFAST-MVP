...
const NotificationAIEngine = {
  personalizeList: ({ notifications }: { notifications: Notification[] }) => notifications,
  formatLiveNotification: ({ notification }: { notification: Notification }): Notification => ({ ...notification, showToast: true, soundEnabled: true }),
  buildToastFromNotification: (n: Notification): Notification => n,
};
const DevicePushRegistration = { registerDevice: async (_user: unknown) => {} };

class LiveNotificationSocket {
  private _listeners: Record<string, (payload: unknown) => void> = {};
  constructor(_user?: unknown) {}
  on(event: string, cb: (payload: unknown) => void): void { this._listeners[event] = cb; }
  connect(): void {}
  disconnect(): void {}
}

const RealtimeBadgeService = {
  broadcastUnreadCount: (_updater: unknown) => {},
};

// ── Sub-components ────────────────────────────────────────────────────────
interface PriorityBadgeProps { score: number; }
function PriorityBadge({ score }: PriorityBadgeProps) {
...
function ChannelBadge({ channel }: ChannelBadgeProps) {
  const labels: Record<string, string> = { push: "📱", email: "📧", sms: "📲", in_app: "🔔" };
  return (
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-slate-400 mt-0.5 line-clamp-3">{n.message}</p>
...
// ── Live toast ────────────────────────────────────────────────────────────
interface LiveToastProps { toast: Notification | null; onClose: () => void; }
function LiveToast({ toast, onClose }: LiveToastProps) {
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-slate-400 line-clamp-3">{toast.message}</p>
        </div>
...
function NotificationsCenter() {
  ...
  const socketRef  = useRef<LiveNotificationSocket | null>(null);
  ...
  useEffect(() => {
    ...
    socket.on("notification:new", (payload) => {
      ...
      if (!personalized.isRead) {
        ...
        RealtimeBadgeService.broadcastUnreadCount((prev: number) => prev + 1);
      }
    ...
    socket.on("badge:update", (count) => {
      ...
      RealtimeBadgeService.broadcastUnreadCount(() => count as number);
    });
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const resData = res.data as { data?: { notifications?: Notification[]; stats?: { unreadCount?: number } } } | null;
      const list: Notification[] = resData?.data?.notifications ?? [];
      const personalized = NotificationAIEngine.personalizeList({ notifications: list });
      const filtered = personalized.filter((n) => (n.priorityScore ?? 0) >= (n.minPriority ?? 1.0));
      setNotifications(filtered);
      const uc = resData?.data?.stats?.unreadCount ?? 0;
      setUnreadCount(uc);
      RealtimeBadgeService.broadcastUnreadCount(() => uc);
    } catch (err: unknown) {
      const e = err as { code?: string };
      if (e?.code !== "ERR_CANCELED") console.error(err);
    } finally {
  ...
  const handleMarkAsRead = useCallback(async (id: string) => {
    try {
      ...
      if (mountedRef.current) {
        ...
        RealtimeBadgeService.broadcastUnreadCount((prev: number) => Math.max(0, prev - 1));
      }
