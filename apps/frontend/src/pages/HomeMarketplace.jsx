 ...
 *
 * - Guest: cinematic hero (network‑adaptive video + image fallback) + responsive virtualized grid + stories.
 * - Browser (worker/user/tourist): AI dashboard (Trending/Revenue/Workers/Hotels/Taxi/Restaurants/Apartments/Reviews/Countries),
 *   AI widgets (Weather/Traffic/Currency/Crypto/News/Emergency/Events/Flights/Hotels/Taxi/Jobs/Delivery), live marketplace socket,
 *   Google Maps live tracking (placeholder), offline/PWA, auto theme (Material You / Glass / Luxury).
 * - Provider: dashboard + marketplace finance features (Auction/Bidding/Escrow/Insurance/Refund/Subscription/Coupons/Promo/Loyalty/Membership).
 *
 * NOTE:
 * - Hero video: chooses source by network speed + screen size (adaptive ladder 1080p → 720p → 480p → image fallback).
 * - Socket: SocketManager with reconnect, heartbeat, offline queue, compression flags.
 * - Search: AI / semantic / vector / voice / image / QR / Google Lens hooks (frontend entry points).
 * - Maps: Google Maps-ready container for street view, traffic, cluster, heatmap, live tracking, ETA, offline.
 * - AI: model router (GPT / Gemini / Claude / Mistral / Llama) placeholder for backend function.
 * - Theme: auto dark/light, dynamic color, Material You, glassmorphism, luxury profiles.
 * - Security: biometric hooks (Fingerprint/FaceID/Passkey/Device Binding/Jailbreak/Root detection/session rotation) – needs native/bridge.
 * - Performance: React Query/TanStack Query, persisted cache, service worker, background sync, image CDN (AVIF/WebP), edge cache.
 * - Accessibility: screen reader labels, keyboard navigation, high contrast, dynamic font size, voice control hooks.
 */
  ...
  return (
    <img
      ...
      className={`${className} transition-opacity duration-700 ${
        loaded ? "opacity-100" : "opacity-70 blur-sm"
      }`}
    />
  );
});
...
function useAutoTheme(userPreference) {
...
// ───────────────── Security hooks (skeleton) ──────────────────
// NOTE: these need native/SDK integration (browser + mobile).
function useSecurityHardening() {
  useEffect(() => {
    // Fingerprint / Device binding (deviceId from local storage / secure enclave).
    // Passkey (WebAuthn) setup.
    // Session rotation (short-lived tokens + refresh).
    // Jailbreak/Root detection only on mobile native bridge.
    // Encryption hooks (e.g. Crypto.subtle for sensitive local data).
  }, []);
}

// ───────────────── Marketplace finance hooks (skeleton) ──────
function useMarketplaceFinance() {
  // Auction/Bidding/Escrow/Insurance/Refund/Subscription/Coupons/Promo/Loyalty/Membership
  // These are domain/business features, here we only expose flags & configs.
  return {
    auctionEnabled: true,
    biddingEnabled: true,
    escrowEnabled: true,
...
const CategoryCard = memo(function CategoryCard({ config, stats, onClick }) {
  const imageSrc =
    config.heroImage || `/assets/categories/${config.role}-4k.webp`;
  const blurSrc =
    config.heroBlur || `/assets/categories/${config.role}-blur.jpg`;

  const themeColors = {
    hotel: "from-blue-500/40",
    restaurant: "from-orange-500/40",
    taxi: "from-yellow-500/40",
    hospital: "from-green-500/40",
    construction: "from-amber-700/40",
    default: "from-sky-500/40",
  };
  const glow = themeColors[config.role] || themeColors.default;
...
// ───────────────── AI Concierge Button & Panel ────────────────
const AIAssistant = memo(function AIAssistant({ onOpen }) {
  return (
    <motion.button
      type="button"
      onClick={onOpen}
...
function AIAssistantPanel({ open, onClose, user, onRunQuery }) {
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] text-slate-300">
                AI Concierge –{" "}
                {t("marketplace.ai.hello", { name: user?.name || "Guest" })}
              </p>
              <button
                type="button"
                onClick={onClose}
            ...
            <p className="text-[11px] text-slate-200 mb-2">
              Egzanp: “Mwen bezwen yon hotel bò plaj”.
            </p>
            <textarea
              rows={3}
              value={prompt}
            ...
            <div className="mt-3 flex justify-end gap-2">
              <button
                ...
                className="px-3 py-2 rounded-xl bg-emerald-500 text-[11px] font-bold text-slate-950"
              >
                Rechèch
              </button>
            </div>
...
// ───────────────── Hero video (network‑adaptive) ──────────────
function PremiumHero({ roleLabel, metrics, persona }) {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    ...
    const testUrl = "/assets/network-test.jpg";
    const start = performance.now();
    const img = new Image();
    img.src = testUrl;
    img.onload = () => {
      const duration = performance.now() - start;
      // If very slow (> 1500ms), go down to 480p, else choose 720/1080 by width.
      if (duration > 1500) {
        setVideoSrc(HERO_VIDEO_SRC_480);
      } else if (width <= 768) {
        setVideoSrc(HERO_VIDEO_SRC_720);
      } else {
        setVideoSrc(HERO_VIDEO_SRC_1080);
      }
    };
    img.onerror = () => {
      // If test fails, use image fallback for less battery/data.
      setVideoError(true);
    };
  }, []);
  ...
  const heroThemeBg =
    roleLabel === "hotel"
      ...
      ? "bg-gradient-to-br from-orange-900 via-slate-900 to-black"
      : roleLabel === "taxi"
      ? "bg-gradient-to-br from-yellow-700 via-slate-900 to-black"
      : roleLabel === "hospital"
      ? "bg-gradient-to-br from-green-900 via-slate-900 to-black"
      : roleLabel === "construction"
      ? "bg-gradient-to-br from-amber-900 via-slate-900 to-black"
      : "bg-[#020617]";
  ...
  return (
    ...
    >
      ...
      {/* Glass/3D dashboard overlay */}
      <div className="pointer-events-none absolute inset-0">
        <motion.div
          className="absolute -top-10 left-10 h-24 w-24 rounded-full bg-amber-500/10 blur-3xl"
          animate={{ x: [0, 20, -10, 0], y: [0, -10, 20, 0] }}
          transition={{ duration: 14, repeat: Infinity }}
        />
        <motion.div
          className="absolute bottom-0 right-0 h-32 w-32 rounded-full bg-sky-500/10 blur-3xl"
      ...
      <div className="relative z-10 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[11px] text-slate-300 flex items-center gap-1">
              ...
              {t("marketplace.hero.greeting", {
                name: metrics.name || "Guest",
              })}
            </p>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-[10px]">
          {[
            ...
            <motion.button
              ...
              type="button"
              whileTap={{ scale: 0.95 }}
              className="rounded-xl bg-black/40 px-3 py-1.5 text-slate-200 border border-slate-700/60 hover:border-amber-400/60 hover:text-amber-300"
            >
              {label}
            </motion.button>
          ))}
        ...
        {/* Animated metrics */}
        <motion.div
          className="grid grid-cols-2 gap-3 sm:grid-cols-4 mt-3"
        ...
        >
          {[
            ...
            <div
              key={item.label}
              className="flex items-center gap-2 rounded-2xl bg-black/45 px-3 py-2 border border-slate-800/70 shadow-[0_10px_30px_rgba(15,23,42,0.85)]"
            >
        ...
        {/* Apple Vision Pro style 3D widgets */}
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] mb-2">
        <button className="flex items-center gap-1 rounded-2xl bg-[#111827] px-3 py-2 text-slate-200">
          <span aria-hidden="true">🔍</span> {t("marketplace.search.search")}
        </button>
        <button className="flex items-center gap-1 rounded-2xl bg-[#111827] px-3 py-2 text-slate-200">
          <span aria-hidden="true">📍</span> {t("marketplace.search.where")}
        </button>
        <button className="flex items-center gap-1 rounded-2xl bg-[#111827] px-3 py-2 text-slate-200">
          <span aria-hidden="true">📅</span> {t("marketplace.search.date")}
        </button>
        <button className="flex items-center gap-1 rounded-2xl bg-[#111827] px-3 py-2 text-slate-200">
          <span aria-hidden="true">👥</span> {t("marketplace.search.people")}
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          type="button"
          aria-label={t("marketplace.search.budget")}
          className="rounded-2xl bg-[#111827] px-3 py-2 text-[11px] text-slate-200"
        >
          💰
        </button>
        <button
          type="button"
          aria-label={t("marketplace.search.rating")}
          className="rounded-2xl bg-[#111827] px-3 py-2 text-[11px] text-slate-200"
        >
          ⭐
        </button>
        <button
          type="button"
          aria-label={t("marketplace.search.voice")}
          onClick={onVoice}
          className="rounded-2xl bg-[#111827] px-3 py-2 text-[11px] text-slate-200"
        >
          🎙
        ...
        <button
          type="button"
          aria-label={t("marketplace.search.ai")}
          onClick={onAISearch}
          className="rounded-2xl bg-amber-500 px-3 py-2 text-[11px] font-bold text-slate-950"
        >
          🤖 AI
        </button>
      </div>
...
function MapPreview() {
  return (
    ...
    >
      {/* Placeholder: Google Maps JS API container */}
      <div id="jobfast-map-container" className="w-full h-full" />
      <p className="absolute top-2 left-3 text-[10px] font-bold text-slate-300">
        Google Maps (live tracking, traffic, ETA)
      </p>
      <p className="absolute bottom-2 left-3 text-[9px] text-slate-400">
        Street View, cluster markers, heatmap, offline tiles, navigation, etc.
      </p>
    </motion.div>
  );
}
    ...
    >
      <div className="flex items-center gap-3">
        <div className="h-14 w-14 rounded-2xl overflow-hidden border border-amber-400/60">
          <ProgressiveImage
            src="/assets/featured/hotel-puntacana-4k.webp"
            placeholder="/assets/featured/hotel-puntacana-blur.jpg"
            alt="Best Hotel in Punta Cana"
            className="h-full w-full object-cover"
          />
        </div>
        <div className="min-w-0">
...
function LiveMarketplaceStrip({ live }) {
  const { t } = useTranslation();
  ...
  return (
    <motion.div
      className="mt-3 rounded-2xl border border-emerald-500/30 bg-emerald-900/20 px-4 py-3"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
    >
...
// ───────────────── Story strip ────────────────────────────────
const StoryStrip = memo(function StoryStrip({ stories }) {
            ...
            <div className="px-2 py-1">
              ...
              {story.liveWorker && (
                <p className="text-[8px] text-slate-300 mt-0.5">
                  👷 {story.liveWorker.name} – {story.liveWorker.distance} ·{" "}
                  {story.liveWorker.rating}★
                </p>
              )}
            </div>
          </motion.button>
