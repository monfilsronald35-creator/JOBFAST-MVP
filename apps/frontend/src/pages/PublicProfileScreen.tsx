...
type PillTone   = 'slate' | 'emerald' | 'amber' | 'sky' | 'rose' | 'violet';

interface LanguageItem    { name: string; level: number }
interface SkillItem       { name: string; level: number }
interface Achievement     { id: string; label: string; icon: LucideIcon; tone: string }
interface ExperienceItem  { company: string; role: string; period: string; current: boolean }
interface EducationItem   { school: string; degree: string; year: string }
interface ServiceItem     { name: string; price: string; icon: string }
interface PortfolioItem   { id: string; type: string; src?: string; label: string; alt?: string; poster?: string; before?: string; after?: string }
interface ReviewItem      { id: string; author: string; rating: number; text: string; date: string }
interface TimelineItem    { id: string; year: string; title: string; detail: string }

interface ProfileRating {
  overall: number; quality: number; communication: number;
  speed: number; professionalism: number; cleanliness: number;
  respect: number; safety: number;
}
interface ProfileStats {
  visitorsToday: number; visitorsWeek: number; visitorsMonth: number;
  searchAppearances: number; clicks: number; messages: number;
  bookings: number; hires: number; jobs: number; projects: number;
  clients: number; followers: number; following: number; views: number;
  responseRate: number; responseTime: string; repeatClients: number; hiringSuccess: number;
}
interface Profile {
  id: string;
  name: string;
  ...
  paymentTrust: Record<string, boolean>;
  trustScore: number;
  memberSince: string;
  joinedAt: string;
  badges: string[];
  ...
  heatMap: string[];
  aiMatch: { score: number; reasons: string[] };
  aiRecs: string[];
}

const cls    = (...p: (string | undefined | false | null)[]): string => p.filter(Boolean).join(' ');
const fmtNum = (n: number | string | undefined): string => {
  const v = Number(n || 0);
...
const stars = (n: number): string =>
  '★★★★★☆☆☆☆☆'.slice(5 - Math.max(0, Math.min(5, Math.round(n))), 10 - Math.max(0, Math.min(5, Math.round(n))));

function clamp(n: number, min: number, max: number): number {
  return Math.min(max, Math.max(min, n));
}

function useInView(ref: React.RefObject<HTMLElement | null>, rootMargin = '220px'): boolean {
...
function useDeferredSection(delay = 0): { ref: React.RefObject<HTMLDivElement | null>; ready: boolean } {
...
interface GlassCardProps { children: React.ReactNode; className?: string; glow?: boolean }
const GlassCard = memo(function GlassCard({ children, className = '', glow = false }: GlassCardProps) {
  return (
    <div
      className={cls(
...
const SkeletonBlock = ({ className = '' }: { className?: string }) => (
  <div className={cls('animate-pulse rounded-2xl bg-white/8', className)} />
);

interface SectionTitleProps { children: React.ReactNode; right?: React.ReactNode }
const SectionTitle = memo(function SectionTitle({ children, right }: SectionTitleProps) {
  return (
...
interface PillProps { children: React.ReactNode; tone?: PillTone }
...
interface StatBoxProps { value: number | string; label: string; icon?: LucideIcon }
const StatBox = memo(function StatBox({ value, label, icon: Icon }: StatBoxProps) {
  return (
...
function buildSkeletonItems(n: number): number[] {
  return Array.from({ length: n }, (_, i) => i);
}

function buildProfile(seed: Partial<Profile> = {}): Profile {
  return {
    id:           seed.id           ?? 'u1',
    ...
    salary:      seed.salary      ?? '$900/semèn',
    contract:    seed.contract    ?? 'Full-time / Contract',
    travelAvail: seed.travelAvail ?? true,
    rating: seed.rating ?? {
      overall: 4.9, quality: 4.9, communication: 4.8, speed: 4.7,
      professionalism: 5.0, cleanliness: 4.9, respect: 5.0, safety: 4.8,
    },
    totalReviews: seed.totalReviews ?? 127,
    stats: seed.stats ?? {
      visitorsToday: 182,    visitorsWeek: 1240,  visitorsMonth: 5490,
      searchAppearances: 23890, clicks: 3920,    messages: 860,
      bookings: 248,         hires: 64,           jobs: 245,
      projects: 38,          clients: 112,        followers: 1840,
      following: 203,        views: 9200,         responseRate: 98,
      responseTime: '3 min', repeatClients: 87,   hiringSuccess: 94,
    },
    verified: seed.verified ?? {
      passport: true, nationalId: true, driverLicense: true, policeRecord: true,
      companyLicense: false, taxNumber: true, identity: true, face: true,
      phone: true, email: true, address: true, company: false,
      background: true, payment: true, contracts: true, noDisputes: true,
    },
    paymentTrust: seed.paymentTrust ?? { escrow: true, stripe: true, paypal: true, bank: true },
    trustScore:  seed.trustScore  ?? 92,
    memberSince: seed.memberSince ?? '2026',
    joinedAt:    seed.joinedAt    ?? 'Joined JOBFAST 2026',
    badges: seed.badges ?? ['Identity Verified', 'Phone Verified', 'Email Verified', 'Background Checked', 'Premium Member'],
    ...
    skills: seed.skills ?? [
      { name: 'Electrical', level: 98 }, { name: 'Painting', level: 94 },
      { name: 'Leadership', level: 89 }, { name: 'Plumbing', level: 91 },
      { name: 'Safety',     level: 97 }, { name: 'Blueprint', level: 88 },
    ],
    ...
    heatMap:  seed.heatMap  ?? ['Punta Cana', 'Bávaro', 'Santo Domingo', 'Higüey'],
    aiMatch:  seed.aiMatch  ?? { score: 95, reasons: ['Experience', 'Distance', 'Verified', 'Languages', 'Rating', 'Availability'] },
    aiRecs:   seed.aiRecs   ?? ['Foreman', 'Project Manager', 'Safety Officer', 'Construction Consultant'],
  };
}
...
function ProgressBar({ value, className = 'bg-amber-400' }: { value: number; className?: string }) {
  return (
...
function HeroSection({ profile, followed, saved, onFollow, onSave, navigate, onMessage }: HeroSectionProps) {
    ...
    <div className="relative">
      <motion.div style={{ y: coverY }} className="relative h-[380px] overflow-hidden rounded-[2rem] md:h-[460px]">
        <video
          ...
          autoPlay muted loop playsInline poster={profile.introPoster}
        >
              ...
              <div className="flex items-end gap-4">
                <motion.div style={{ scale: avatarScale, y: avatarY }} className="relative shrink-0">
                  ...
                  <motion.div
                    ...
                    className="absolute -right-1 -bottom-1 flex h-7 w-7 items-center justify-center rounded-full border border-black/30 bg-emerald-400 shadow-[0_0_20px_rgba(34,197,94,.55)]"
                  >
                ...
                <motion.div style={{ y: nameY }} className="pb-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <motion.span
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 text-[10px] font-black text-emerald-300"
                    >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-slate-300">
                    <Pill tone={statusTone}>
                      {profile.liveStatus === 'available' ? <Wifi className="h-3.5 w-3.5" />
                        : profile.liveStatus === 'busy' ? <TimerReset className="h-3.5 w-3.5" />
...
function LivePresenceCard({ profile }: { profile: Profile }) {
  return (
    <GlassCard>
      <div className="p-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="mt-1 text-xl font-black text-white">{profile.lastSeen}</div>
          </div>
        </div>
      </div>
    </GlassCard>
...
function DistanceCard({ profile }: { profile: Profile }) {
  return (
    <GlassCard>
      <div className="p-4">
        <div className="flex items-center gap-2">
...
function AIResumeActions() {
  type ActionTuple = [string, LucideIcon, string];
        ...
        <div className="grid gap-2 md:grid-cols-4">
          {actions.map(([label, Icon, tone]) => (
            <button key={label} className={cls(
              'glass-action justify-center',
              tone === 'emerald' && 'border-emerald-400/20',
              tone === 'sky'     && 'border-sky-400/20',
              tone === 'amber'   && 'border-amber-400/20',
              tone === 'violet'  && 'border-violet-400/20',
            )}>
...
function ReputationEngine({ profile }: { profile: Profile }) {
  type MetricTuple = [string, boolean | string, string];
  const metrics: MetricTuple[] = [
    ['No disputes',      profile.verified['noDisputes'] ?? false, 'emerald'],
          ...
          {metrics.map(([label, value, tone]) => (
            <div key={label} className="rounded-2xl border border-white/10 bg-white/5 p-3">
              ...
              <div className={cls(
                ...
                tone === 'emerald' ? 'text-emerald-300' : tone === 'rose' ? 'text-rose-300'
                  : tone === 'amber' ? 'text-amber-300' : tone === 'sky' ? 'text-sky-300' : 'text-violet-300'
              )}>
...
function GlobalVerification({ profile }: { profile: Profile }) {
  const items: [string, boolean][] = [
    ['Passport',        profile.verified['passport']       ?? false],
    ['National ID',     profile.verified['nationalId']     ?? false],
    ['Driver License',  profile.verified['driverLicense']  ?? false],
    ['Police Record',   profile.verified['policeRecord']   ?? false],
    ['Company License', profile.verified['companyLicense'] ?? false],
    ['Tax Number',      profile.verified['taxNumber']      ?? false],
  ];
  return (
    <GlassCard>
      <div className="p-4">
...
function PaymentTrust({ profile }: { profile: Profile }) {
  const items: [string, boolean][] = [
    ['Escrow Enabled', profile.paymentTrust['escrow'] ?? false],
    ['Stripe Verified', profile.paymentTrust['stripe'] ?? false],
    ['PayPal Verified', profile.paymentTrust['paypal'] ?? false],
    ['Bank Verified',   profile.paymentTrust['bank']   ?? false],
  ];
  return (
    <GlassCard>
      <div className="p-4">
        ...
        <div className="space-y-3">
          {profile.skills.map(skill => (
            <div key={skill.name} className="rounded-2xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="mt-2">
                <ProgressBar value={skill.level} className="bg-gradient-to-r from-amber-400 via-yellow-300 to-emerald-400" />
              </div>
            </div>
          ))}
        </div>
...
export default function PublicProfileScreen() {
  ...
  const followUserFn   = followUser   as unknown as (id: string) => void;
  const unfollowUserFn = unfollowUser as unknown as (id: string) => void;
  const isFollowingFn  = isFollowing  as unknown as (id: string) => boolean;
  const saveUserFn     = saveUser     as unknown as (id: string) => void;
  const unsaveUserFn   = unsaveUser   as unknown as (id: string) => void;
  const isSavedFn      = isSaved      as unknown as (id: string) => boolean;
  ...
  useEffect(() => {
    ...
    API.get(`/profiles/${userId}/public`)
      .then((res: { data: Profile }) => {
        if (cancelled) return;
        setProfile(res.data);
        setFollowed(isFollowingFn(userId));
        setSaved(isSavedFn(userId));
      })
      .catch((err: unknown) => {
        if (cancelled) return;
        const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
        setError(msg ?? 'Profile not found');
      })
  ...
  const handleFollow = useCallback(() => {
    ...
    if (followed) { unfollowUserFn(userId ?? ''); setFollowed(false); }
    else           { followUserFn(userId ?? '');   setFollowed(true); }
  }, [followed, userId, authUser, navigate]);

  const handleSave = useCallback(() => {
    if (!authUser) { navigate('/login'); return; }
    if (saved) { unsaveUserFn(userId ?? ''); setSaved(false); }
    else        { saveUserFn(userId ?? '');   setSaved(true); }
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: BG }}>
      ...
      <div className="px-4 max-w-2xl mx-auto space-y-4 mt-4">
        ...
        {(profile.skills?.length ?? 0) > 0 && <SkillsSection profile={profile} />}
        {(profile.languages?.length ?? 0) > 0 && <LanguagesSection profile={profile} />}
