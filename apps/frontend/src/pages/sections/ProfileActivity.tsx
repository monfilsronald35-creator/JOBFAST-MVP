...
interface Activity { id?: string; icon?: string; title?: string; time?: string; }
interface ActivityData { items?: Activity[]; [key: string]: unknown; }
interface SectionProps { profile?: Record<string, unknown>; data?: ActivityData | Activity[]; }

const ProfileActivity = memo(function ProfileActivity({ profile: _profile, data }: SectionProps) {
  const activities: Activity[] = Array.isArray(data) ? data : (data?.items ?? []);
  return (
