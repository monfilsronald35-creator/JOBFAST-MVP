...
interface BookingEntry { id?: string; title?: string; date?: string; duration?: string; status?: string; }
interface BookingData { bookings?: BookingEntry[]; [key: string]: unknown; }
interface SectionProps { profile?: Record<string, unknown>; data?: BookingData | BookingEntry[]; }

const BookingPanel = memo(function BookingPanel({ profile: _profile, data }: SectionProps) {
  const bookings: BookingEntry[] = Array.isArray(data) ? data : (data?.bookings ?? []);
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button type="button" className="ml-auto rounded-2xl bg-amber-400 px-4 py-1.5 text-[11px] font-bold text-black hover:bg-amber-300 transition-colors">
          Book Now
        </button>
      </div>
      {bookings.length === 0 ? (
