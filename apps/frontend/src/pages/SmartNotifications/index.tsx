...
interface CatStyle { dot: string; bg: string; }

// ── Category color map ─────────────────────────────────────────
const CAT_COLOR: Record<string, CatStyle> = {
...
const CAT_ACTIONS: Record<string, CatAction[]> = {
  jobs: [
...
// ── Count per category ────────────────────────────────────────
function getCounts(notifs: MockNotif[]): Record<string, number> {
...
function NotifRow({ notif, onRead, navigate }: NotifRowProps) {
  ...
  function handleAction(e: React.MouseEvent, action: CatAction) {
    e.stopPropagation();
    if (action.toggle) {
...
export default function SmartNotifications() {
  ...
  const [notifs,    setNotifs]    = useState<MockNotif[]>(MOCK_NOTIFS);
