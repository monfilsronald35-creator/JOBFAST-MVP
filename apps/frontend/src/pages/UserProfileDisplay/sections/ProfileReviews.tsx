...
interface ReviewData { reviews?: Review[]; averageRating?: number; totalCount?: number; }
interface ProfileData { rating?: number; reviewCount?: number; [key: string]: unknown; }
interface SectionProps { profile?: ProfileData; data?: ReviewData | Review[]; }

function StarRating({ value = 0 }: { value?: number }) {
  return <span className="text-amber-400 text-[11px]">{'★'.repeat(Math.round(value))}{'☆'.repeat(5 - Math.round(value))}</span>;
}

const ProfileReviews = memo(function ProfileReviews({ profile, data }: SectionProps) {
  ...
  const total = (!Array.isArray(data) ? data?.totalCount : null) ?? profile?.reviewCount ?? reviews.length;
