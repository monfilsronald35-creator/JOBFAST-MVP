...
interface ProfileData { trustScore?: number; verificationStatus?: string; [key: string]: unknown; }
interface TrustData { trustScore?: number; verifiedFields?: string[]; [key: string]: unknown; }
interface SectionProps { profile?: ProfileData; data?: TrustData; }

const ProfileTrust = memo(function ProfileTrust({ profile, data }: SectionProps) {
  const score = data?.trustScore ?? profile?.trustScore ?? 0;
  const verified: string[] = data?.verifiedFields ?? [];
    ...
    <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {BADGES.map(({ key, label, icon }) => {
          ...
          return (
            <div key={key} className={`flex items-center gap-2 rounded-2xl px-3 py-2 text-[11px] border ${
              isVerified ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300'
