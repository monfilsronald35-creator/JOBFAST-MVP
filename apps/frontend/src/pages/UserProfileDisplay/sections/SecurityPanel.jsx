...
const SecurityPanel = memo(function SecurityPanel({ profile }) {
  return (
    <motion.section
      ...
      <ul className="space-y-2">
        {SECURITY_ITEMS.map(({ label, status, icon }) => (
          <li key={label} className="flex items-center justify-between rounded-2xl border border-white/8 bg-black/20 px-3 py-2">
            <span className="text-[11px] text-slate-200 flex items-center gap-2">
              <span>{icon}</span>
              {label}
            </span>
            <span className="text-[9px] font-bold text-emerald-300 bg-emerald-400/10 border border-emerald-400/30 px-2 py-0.5 rounded-full">
              {status}
            </span>
          </li>
        ))}
