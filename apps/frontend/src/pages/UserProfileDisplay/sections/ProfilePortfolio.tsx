...
interface PortfolioItem { id?: string; thumbnail?: string; title?: string; }
interface PortfolioData { items?: PortfolioItem[]; [key: string]: unknown; }
interface SectionProps { profile?: Record<string, unknown>; data?: PortfolioData | PortfolioItem[]; }

const ProfilePortfolio = memo(function ProfilePortfolio({ profile: _profile, data }: SectionProps) {
  const items: PortfolioItem[] = Array.isArray(data) ? data : (data?.items ?? []);
  return (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {items.map((item, i) => (
            <div key={item.id ?? i} className="relative overflow-hidden rounded-2xl aspect-square bg-slate-800">
              {item.thumbnail && (
                <img src={item.thumbnail} alt={item.title ?? 'Portfolio item'}
