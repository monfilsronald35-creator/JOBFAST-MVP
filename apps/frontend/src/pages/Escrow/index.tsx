...
import {
  Shield, ShieldAlert, Clock3, FileText,
  Image as ImageIcon, Video, Paperclip, Search,
  MoreVertical, X, Upload, Signature, Send, Sparkles,
  CircleCheckBig, Wallet, Landmark, Briefcase, Store,
  Hotel, Home, CarFront, Construction, Package, LandPlot,
  AlertTriangle, Check, ArrowRightLeft, Clock4, BadgeCheck,
  FileUp, ClipboardCheck, Scale, Bot, Users,
} from 'lucide-react';
...
interface DisputePayload { reason: string; details: string }
interface UploadPayload  { files: File[]; caption: string }

// ── Mock data ──────────────────────────────────────────────────

const MOCK_ESCROWS: EscrowRecord[] = [
  { _id: 'e1', jobTitle: 'Marketplace Product Escrow', category: 'marketplace', amount: 45000, currency: 'HTG', employerName: 'Konstriksyon HAÏTI SA', workerName: 'Jean-Pierre M.', status: 'in_progress', myRole: 'worker', startDate: '2026-07-08', dueDate: '2026-07-20', endDate: '2026-07-20', autoReleaseDays: 7, releaseMode: 'milestone', releasePercent: 30, workerValidated: false, employerValidated: false, milestoneIndex: 1, milestoneTotal: 3, proofCount: 2, description: 'Enstalasyon elektrik konplè pou batiman 3 nivo nan Pétion-Ville.', disputeReason: '', evidence: [{ type: 'photo', name: 'site-photo-1.jpg' }, { type: 'doc', name: 'invoice.pdf' }], signatures: { employer: false, worker: false } },
  { _id: 'e2', jobTitle: 'Hotel Reservation Escrow', category: 'reservation', amount: 18000, currency: 'HTG', employerName: 'Marie Dupont', workerName: 'Anselme T.', status: 'pending_validation', myRole: 'employer', startDate: '2026-07-01', dueDate: '2026-07-10', endDate: '2026-07-10', autoReleaseDays: 3, releaseMode: 'partial', releasePercent: 50, workerValidated: true, employerValidated: false, milestoneIndex: 2, milestoneTotal: 2, proofCount: 1, description: 'Penti konplè 5 chanm nan rezidans Bourdon.', disputeReason: '', evidence: [{ type: 'video', name: 'finish-video.mp4' }], signatures: { employer: true, worker: true } },
  { _id: 'e3', jobTitle: 'Freelance Service Escrow', category: 'freelance', amount: 12000, currency: 'HTG', employerName: 'Résidence Belle Vue', workerName: 'Calixte B.', status: 'completed', myRole: 'worker', startDate: '2026-06-25', dueDate: '2026-07-02', endDate: '2026-07-02', autoReleaseDays: 1, releaseMode: 'full', releasePercent: 100, workerValidated: true, employerValidated: true, milestoneIndex: 3, milestoneTotal: 3, proofCount: 3, description: 'Travay plonbri nan 2 saldeben.', disputeReason: '', evidence: [{ type: 'photo', name: 'before.jpg' }, { type: 'photo', name: 'after.jpg' }], signatures: { employer: true, worker: true } },
  { _id: 'e4', jobTitle: 'Construction Project Escrow', category: 'construction', amount: 35000, currency: 'HTG', employerName: 'Jean-Robert P.', workerName: 'Délira C.', status: 'disputed', myRole: 'employer', startDate: '2026-06-20', dueDate: '2026-07-05', endDate: '2026-07-05', autoReleaseDays: 14, releaseMode: 'milestone', releasePercent: 30, workerValidated: true, employerValidated: false, milestoneIndex: 1, milestoneTotal: 4, proofCount: 4, description: 'Konstriksyon twati lakay 4 chanm.', disputeReason: 'Travay la pa fini jan yo te konvni a.', evidence: [{ type: 'photo', name: 'roof-stage.jpg' }, { type: 'doc', name: 'contract.pdf' }], signatures: { employer: true, worker: false } },
];
...
const FALLBACK_STATUS: StatusConfig = STATUS_CFG['funded']!;
...
const fmt    = (amount: number | string, currency = 'HTG'): string => `${currency} ${Number(amount).toLocaleString()}`;
const fmtPct = (n: number | string): string => `${Number(n || 0)}%`;

type BadgeTone = 'slate' | 'blue' | 'green' | 'amber' | 'red' | 'indigo' | 'cyan' | 'violet';
...
function Badge({ children, tone = 'slate' }: { children: React.ReactNode; tone?: BadgeTone }) {
  ...
  return <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full border text-[10px] font-bold ${tones[tone]}`}>{children}</span>;
}

function calcProgress(escrow: EscrowRecord): number {
  const total   = Number(escrow.milestoneTotal  || 1);
  const current = Number(escrow.milestoneIndex  || 0);
  return Math.min(100, Math.round((current / total) * 100));
}

function canAutoRelease(escrow: EscrowRecord): boolean {
...
function getEvidenceIcon(type: string): React.ComponentType<{ className?: string }> {
...
interface EscrowMaps {
  escrowMap: Map<string, EscrowRecord>;
  roleMap: { worker: string[]; employer: string[] };
}

function useEscrowMaps(escrows: EscrowRecord[]): EscrowMaps {
  return useMemo(() => {
    const byId: Map<string, EscrowRecord> = new Map();
    const byRole: { worker: string[]; employer: string[] } = { worker: [], employer: [] };
...
const EvidenceList = memo(function EvidenceList({ items = [], t }: EvidenceListProps) {
  return (
...
interface DisputeModalProps {
  escrow: EscrowRecord;
  onClose: () => void;
  onDispute: (id: string, payload: DisputePayload) => Promise<void> | void;
}

function DisputeModal({ escrow, onClose, onDispute }: DisputeModalProps) {
  ...
  return (
    <BottomSheet open title={t('escrow.openDispute', { defaultValue: 'Ouvri Dispute' })} onClose={onClose}>
      ...
      <form onSubmit={e => void submit(e)} className="space-y-3">
        <textarea value={reason} onChange={e => setReason(e.target.value)} rows={3} required placeholder={t('escrow.disputeReason', { defaultValue: 'Esplike pwoblèm nan…' })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-red-500/50 resize-none" />
        <textarea value={details} onChange={e => setDetails(e.target.value)} rows={3} placeholder={t('escrow.disputeDetails', { defaultValue: 'Ajoute plis detay, nimewo dosye, oswa nòt medyatè.' })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-red-500/50 resize-none" />
...
interface EvidenceUploadModalProps {
  escrow: EscrowRecord;
  onClose: () => void;
  onUpload: (id: string, payload: UploadPayload) => Promise<void> | void;
}

function EvidenceUploadModal({ escrow, onClose, onUpload }: EvidenceUploadModalProps) {
  const { t } = useTranslation();
  ...
  return (
    <BottomSheet open title={t('escrow.uploadEvidenceTitle', { defaultValue: 'Upload prèv' })} onClose={onClose}>
      ...
      <form onSubmit={e => void submit(e)} className="space-y-3">
        <input type="file" multiple onChange={e => setFiles(Array.from(e.target.files ?? []))} className="w-full text-xs text-slate-300" />
        <textarea value={caption} onChange={e => setCaption(e.target.value)} rows={3} placeholder={t('escrow.evidenceCaption', { defaultValue: 'Kisa prèv yo montre?' })} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 resize-none" />
...
interface ActionsSheetProps {
  ...
  onDispute: (id: string, payload: DisputePayload) => Promise<void> | void;
  onAutoRelease: (id: string) => Promise<void> | void;
  onPartialRelease: (id: string, percent: number) => Promise<void> | void;
  onMilestoneAdvance: (id: string) => Promise<void> | void;
  onUploadEvidence: (id: string) => void;
  onSign: (id: string) => Promise<void> | void;
}

function ActionsSheet({ escrow, open, onClose, onValidate, onDispute, onAutoRelease, onPartialRelease, onMilestoneAdvance, onUploadEvidence, onSign }: ActionsSheetProps) {
  const { t } = useTranslation();
  ...
  return (
    <BottomSheet open={open} title={t('escrow.moreActions', { defaultValue: 'More Actions' })} onClose={onClose}>
      <div className="space-y-2">
        ...
        <button type="button" onClick={() => { onClose(); void onPartialRelease(escrow._id, 30); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-800/70 border border-slate-700">
        ...
        <button type="button" onClick={() => { onClose(); void onPartialRelease(escrow._id, 50); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-800/70 border border-slate-700">
        ...
        <button type="button" onClick={() => { onClose(); void onPartialRelease(escrow._id, 100); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-800/70 border border-slate-700">
        ...
        <button type="button" onClick={() => { onClose(); void onMilestoneAdvance(escrow._id); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-800/70 border border-slate-700">
        ...
        <button type="button" onClick={() => { onClose(); void onSign(escrow._id); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-800/70 border border-slate-700">
        ...
        {canAuto && (
          <button type="button" onClick={() => { onClose(); void onAutoRelease(escrow._id); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30">
        ...
        <button type="button" onClick={() => { onClose(); void onValidate(escrow._id); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-green-500/10 border border-green-500/30">
        ...
        <button type="button" onClick={() => { onClose(); void onDispute(escrow._id, { reason: t('escrow.disputeQuickReason', { defaultValue: 'Mwen vle ouvri yon dispute.' }), details: '' }); }} className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-red-500/10 border border-red-500/30">
...
function SignatureState({ label, done, t }: { label: string; done: boolean; t: (key: string, opts?: Record<string, unknown>) => string }) {
  return (
...
function EscrowCard({ escrow, onClick }: { escrow: EscrowRecord; onClick: () => void }) {
  const { t } = useTranslation();
  const cfg      = STATUS_CFG[escrow.status] ?? FALLBACK_STATUS;
  const category = CATEGORY_CFG[escrow.category] ?? null;
  ...
  return (
    <button type="button" onClick={onClick} className="w-full text-left bg-[#0d1526] border border-slate-800 hover:border-slate-700 rounded-2xl overflow-hidden transition">
      <div className="flex items-center gap-3 p-4">
        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-xl border shrink-0 ${cfg.bg}`}>{cfg.icon}</div>
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <Badge tone={escrow.status === 'completed' ? 'green' : 'amber'}>{progress}%</Badge>
          </div>
        </div>
...
function EscrowDetail({ escrow, onClose, onValidate, onDispute, onAutoRelease, onPartialRelease, onMilestoneAdvance, onOpenEvidence, onSign, onOpenActions }: EscrowDetailProps) {
  const { t } = useTranslation();
  const cfg          = STATUS_CFG[escrow.status] ?? FALLBACK_STATUS;
  const isWorker     = escrow.myRole === 'worker';
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-[#020617]">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 pb-28">
        ...
        <div className="p-4 bg-[#0d1526] border border-slate-700 rounded-2xl">
          ...
          <div className="space-y-2">
            <SignatureState label={t('escrow.employerSignature', { defaultValue: 'Siyati Anplwayè' })} done={escrow.signatures?.employer ?? false} t={t} />
            <SignatureState label={t('escrow.workerSignature',   { defaultValue: 'Siyati Travayè' })}  done={escrow.signatures?.worker   ?? false} t={t} />
          </div>
        </div>
...
export default function EscrowPage() {
  ...
  const [detail,      setDetail]      = useState<EscrowRecord | null>(null);
  ...
  useEffect(() => {
    setLoading(true);
    (escrowAPI as unknown as { getMyEscrows?: () => Promise<{ data?: { data?: EscrowRecord[] } | EscrowRecord[] }> })
      .getMyEscrows?.()
      .then(res => {
        const d = (res?.data as { data?: EscrowRecord[] } | undefined)?.data ?? (Array.isArray(res?.data) ? res.data as EscrowRecord[] : null);
  ...
  const updateEscrow = useCallback((id: string, updater: (e: EscrowRecord) => EscrowRecord) => {
    setEscrows(prev => prev.map(e => (e._id === id ? updater(e) : e)));
  }, []);

  const syncDetail = useCallback((id: string) => {
    setDetail(prev => {
  ...
  const handleValidate = useCallback(async (id: string) => {
    try { await (escrowAPI as unknown as { validate?: (id: string) => Promise<unknown> }).validate?.(id); } catch {}
    updateEscrow(id, e => {
  ...
  const handleDispute = useCallback(async (id: string, payload: DisputePayload) => {
    try { await (escrowAPI as unknown as { dispute?: (id: string, p: DisputePayload) => Promise<unknown> }).dispute?.(id, payload); } catch {}
    updateEscrow(id, e => ({ ...e, status: 'disputed', disputeReason: payload.reason }));
    syncDetail(id);
  }, [updateEscrow, syncDetail]);

  const handleAutoRelease = useCallback(async (id: string) => {
    try { await (escrowAPI as unknown as { autoRelease?: (id: string) => Promise<unknown> }).autoRelease?.(id); } catch {}
    updateEscrow(id, e => ({ ...e, status: 'completed' }));
    syncDetail(id);
  }, [updateEscrow, syncDetail]);

  const handlePartialRelease = useCallback(async (id: string, percent: number) => {
    try { await (escrowAPI as unknown as { partialRelease?: (id: string, p: { percent: number }) => Promise<unknown> }).partialRelease?.(id, { percent }); } catch {}
    updateEscrow(id, e => ({ ...e, releasePercent: percent, status: percent === 100 ? 'completed' : 'pending_validation' }));
    syncDetail(id);
  }, [updateEscrow, syncDetail]);

  const handleMilestoneAdvance = useCallback(async (id: string) => {
    try { await (escrowAPI as unknown as { advanceMilestone?: (id: string) => Promise<unknown> }).advanceMilestone?.(id); } catch {}
    updateEscrow(id, e => ({ ...e, milestoneIndex: Math.min(e.milestoneIndex + 1, e.milestoneTotal), status: 'in_progress' }));
    syncDetail(id);
  }, [updateEscrow, syncDetail]);

  const handleUploadEvidence = useCallback(async (id: string, payload?: UploadPayload) => {
    try { await (escrowAPI as unknown as { uploadEvidence?: (id: string, p: UploadPayload | undefined) => Promise<unknown> }).uploadEvidence?.(id, payload); } catch {}
    if (!payload) return;
    updateEscrow(id, e => ({
      ...e,
  ...
  const handleSign = useCallback(async (id: string) => {
    try { await (escrowAPI as unknown as { sign?: (id: string) => Promise<unknown> }).sign?.(id); } catch {}
    updateEscrow(id, e => {
      const next: EscrowRecord = { ...e, signatures: { ...(e.signatures ?? { employer: false, worker: false }) } };
  ...
  const enterpriseStats = useMemo(() => {
    const byStatus = (status: string) => escrows.filter(e => e.status === status).length;
    return [
  ...
  const processSteps = [
    { icon: '🏢', label: t('escrow.employer', { defaultValue: 'Anplwayè' }),  isArrow: false },
    { icon: '→',  label: '',                                                   isArrow: true  },
    { icon: '🔒', label: 'JOBFAST\nEscrow',                                    isArrow: false },
    { icon: '→',  label: '',                                                   isArrow: true  },
    { icon: '👷', label: t('escrow.worker', { defaultValue: 'Travayè' }),     isArrow: false },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-[#020617] text-white pb-24">
        ...
        <div className="flex rounded-xl bg-slate-800/60 p-0.5">
          {FILTERS.map(tb => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-[10px] font-bold transition ${activeTab === tb.id ? 'bg-[#0d1526] text-white shadow' : 'text-slate-400'}`}
            >
      ...
      {detailEscrow && (
        <EscrowDetail
          ...
          onOpenEvidence={id => void handleUploadEvidence(id)}
          onSign={handleSign}
