...
// --- Lazy loaded heavy tab sections (code splitting) ---
...
class TabErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    // TODO: send to Sentry / Datadog with tab context
    // console.error('Tab error:', this.props.tabId, error, info);
  }
  handleRetry = () => {
...
// --- Memoized tab wrapper pou limite re-renders ---
const MemoSuspense = React.memo(function MemoSuspense({ fallback, children }) {
...
export default function WorkerProfilePage() {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        if (careerRes.status === 'fulfilled')   setCareerData(careerRes.value.data);
        if (passportRes.status === 'fulfilled') setPassportData(passportRes.value.data);
        ...
        // rejected ones: we just leave null and maybe show partial UI
      } finally {
  ...
  const handleAvailabilityChange = useCallback(async (a) => {
    setAvail(a);
  ...
  // --- AI Dashboard: extended metrics from backend ---
  const aiDashboard = useMemo(() => {
    const d = analyticsData?.aiDashboard || {};
    return {
      missionsToday: d.missionsToday ?? 0,
      recommendedJobs: d.recommendedJobs ?? 0,
      companiesInterested: d.companiesInterested ?? 0,
      contracts: d.contracts ?? 0,
      payments: d.payments ?? 0,
      promotionSignal: d.promotionSignal ?? '—',
      riskSignal: d.riskSignal ?? '—',
      marketSignal: d.marketSignal ?? '—',
      salarySignal: d.salarySignal ?? '—',
      learningSignal: d.learningSignal ?? '—',
      burnoutScore: d.burnoutScore ?? '--',
      healthScore: d.healthScore ?? '--',
      careerPrediction: d.careerPrediction ?? '—',
      futureSalary: d.futureSalary ?? '--',
      retirementProjection: d.retirementProjection ?? '—',
      fraudDetection: d.fraudDetection ?? '—',
      employmentStability: d.employmentStability ?? '—',
      countryRecommendation: d.countryRecommendation ?? '—',
      visaPrediction: d.visaPrediction ?? '—',
      immigrationEligibility: d.immigrationEligibility ?? '—',
      globalDemand: d.globalDemand ?? '—',
      marketForecast: d.marketForecast ?? '—',
      jobReplacementRisk: d.jobReplacementRisk ?? '—',
      skillsForecast: d.skillsForecast ?? '—',
      hiringProbability: d.hiringProbability ?? '—',
    };
  }, [analyticsData]);
  ...
  // --- Tab content is conditionally rendered (inactive tabs unmounted) ---
    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-white pb-28">
      ...
      <div className="fixed inset-0 -z-10 bg-slate-950">
        <div
          ...
          style={{
            backgroundImage: `url(${user?.profileMetadata?.heroBackgroundUrl || '/images/worker-hero-8k.jpg'})`,
          }}
        />
            ...
            <div className="relative">
              ...
              >
                ...
                <motion.div
                  ...
                  animate="animate"
                >
                  <span className={`${AVAIL_CFG[avail]?.color}`}>● {AVAIL_CFG[avail]?.label}</span>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-[10px] text-slate-200">
                    ...
                    <span>{user?.location?.city || user?.location?.country || t('worker.noLocation', { defaultValue:'Lokasyon pa disponib' })}</span>
              ...
              <div className="mt-2 rounded-2xl bg-black/40 border border-white/15 backdrop-blur-xl p-3 text-[10px] text-slate-100">
                <p className="font-semibold mb-1">
                  Good morning {user?.firstName || user?.name || 'Worker'}.
                </p>
                <p className="mb-1">
