...
type TabId = 'services' | 'rates' | 'partners';

interface Service { id: string; label: string; emoji: string; color: string; desc: string; }
interface Rate { from: string; to: string; rate: number; trend: string; icon: string; }
interface Partner { name: string; emoji: string; tagline: string; }
...
const TAB_LABELS: Record<TabId, string> = { services: 'Sèvis', rates: 'Chanj', partners: 'Patnè' };
    ...
    <div className="min-h-screen pb-24" style={{ background: BG }}>
      <div className="px-4 pt-8 pb-4" style={{ background: 'linear-gradient(135deg,#0a1628,#050B18)' }}>
        ...
        <div className="flex gap-1 p-1 rounded-[14px]" style={{ background: 'rgba(255,255,255,.05)' }}>
          {(['services','rates','partners'] as TabId[]).map(t => (
