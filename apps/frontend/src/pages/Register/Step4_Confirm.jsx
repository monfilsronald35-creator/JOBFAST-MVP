...
function formatPhone(phone, defaultCountry = 'US') {
...
function Step4_Confirm({
  ...
  const mutation = useMutation({
    mutationFn: async ({ signal }) => {
      if (!registrationId) {
        const err = new Error(t(`${tKeyPrefix}.missing_registration_id`, { defaultValue: 'Missing registration ID' }));
        err.status = 400;
        throw err;
      }
