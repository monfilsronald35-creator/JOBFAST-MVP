...
type IdleFn = (cb: () => void, opts?: { timeout: number }) => IdleHandle;
type CancelIdleFn = (id: IdleHandle) => void;

const ric: IdleFn =
  typeof window !== 'undefined' &&
  typeof (window as Record<string, unknown>)['requestIdleCallback'] === 'function'
    ? (cb, opts) =>
        (window as unknown as { requestIdleCallback: IdleFn }).requestIdleCallback(cb, opts)
    : (cb) => setTimeout(cb, 300);

const cic: CancelIdleFn =
  typeof window !== 'undefined' &&
  typeof (window as Record<string, unknown>)['cancelIdleCallback'] === 'function'
    ? (id) => (window as unknown as { cancelIdleCallback: CancelIdleFn }).cancelIdleCallback(id)
...
const StepCircle = memo(
  forwardRef<HTMLButtonElement, StepCircleProps>(function StepCircle(
...
interface ColorsType { primary: string; primaryGlow: string; surface: string; surfaceAlt: string; border: string; text: string; mutedText: string; [key: string]: string }
const useThemeTokens = (): ColorsType => useMemo(() => COLORS as unknown as ColorsType, []);

function useAnnouncer(): { message: string; announce: (msg: string) => void } {
...
interface AnalyticsEvent { event: string; data: Record<string, unknown>; ts: number }
interface Analytics { track?: (name: string, payload: Record<string, unknown>) => void }

function useAnalyticsQueue(analytics: Analytics | undefined): (event: string, data?: Record<string, unknown>) => void {
  ...
  const flush = useCallback((): void => {
    ...
    if (analytics?.track) {
      analytics.track('registration_batch', { events: payload as unknown[] });
      return;
    }
  ...
  const schedule = useCallback((): void => {
    if (typeof window !== 'undefined' && typeof (window as Record<string, unknown>)['requestIdleCallback'] === 'function') {
...
export interface RegistrationProgressProps {
  ...
  title?: string;
  stepLabels?: string[];
  stepTextFormatter?: (cur: number, tot: number) => string;
...
const RegistrationProgress = memo(
  function RegistrationProgress({
    ...
    const lang0 = i18n?.language?.split('-')?.[0];
    const localeDir =
      i18n?.dir?.() ??
      (['ar', 'he', 'fa', 'ur'].includes(lang0 ?? '') ? 'rtl' : 'ltr');
    ...
    const resolvedFormatter = stepTextFormatter ?? ((cur: number, tot: number) =>
        ...
        >
          ...
          <div className="relative">
            <div
              ...
              style={{
                borderColor:     colors.border,
                background:      'linear-gradient(90deg, rgba(15,23,42,.85) 0%, rgba(255,255,255,.05) 20%, rgba(15,23,42,.85) 100%)',
              ...
              {Array.from({ length: safeTotal }).map((_, index) => {
                ...
                return (
                  <StepCircle
                    ...
                    onClick={stepHandlers[index] ?? null}
                  />
                );
              })}
            </div>
