...
type AnyStepComponent = React.ComponentType<Record<string, unknown>>;

const STEP_COMPONENTS: Partial<Record<RegistrationStep, AnyStepComponent>> = {
  [RegistrationStep.SELECT_ACCOUNT]: Step0_SelectAccount as AnyStepComponent,
  [RegistrationStep.ROLE]:           Step1_Role          as AnyStepComponent,
  [RegistrationStep.CATEGORY]:       Step2_Category      as AnyStepComponent,
  [RegistrationStep.PROFESSION]:     Step3_Profession    as AnyStepComponent,
  [RegistrationStep.BASIC_INFO]:     Step4_BasicInfo     as AnyStepComponent,
  [RegistrationStep.PROFESSIONAL]:   Step5_Professional  as AnyStepComponent,
  [RegistrationStep.DOCUMENTS]:      Step6_Documents     as AnyStepComponent,
  [RegistrationStep.VERIFICATION]:   Step7_Verification  as AnyStepComponent,
  [RegistrationStep.REVIEW]:         Step8_Review        as AnyStepComponent,
  [RegistrationStep.COMPLETE]:       Step9_Complete      as AnyStepComponent,
};
...
function Register() {
  ...
  const { login: authLogin } = useAuth() as { login: (u: Record<string, unknown>) => void };
  ...
  const progressProps = useMemo(
    () => ({
      ...
      stepTextFormatter: (cur: number, tot: number) =>
        t('registration.stepOf', { cur, tot, defaultValue: `Etap ${cur} sou ${tot}` }),
    }),
    [t, visualStep],
  );
  ...
  const handleComplete = async (): Promise<void> => {
    try {
      const result = await (completeRegistration as () => Promise<{ user?: Record<string, unknown>; token?: string } | undefined>)();
      const user  = result?.user;
      const token = result?.token;
      if (user) {
        authLogin(token ? { ...user, token } : user);
        navigate(getRoleDefaultPath(user['role'] as string));
      } else {
        navigate('/login');
      }
    } catch { }
  ...
  const stepProps = useMemo((): Record<string, unknown> => {
    switch (state.currentStep) {
      case RegistrationStep.SELECT_ACCOUNT:
        return { onSelect: selectAccountType };
      case RegistrationStep.ROLE:
        return { accountType: state.accountType, onSelect: setRoleAndNext };
      case RegistrationStep.CATEGORY:
        return { roleType: state.roleType, onSelect: setCategoryAndNext };
      case RegistrationStep.PROFESSION:
        return { categoryId: state.category, onSelect: setProfessionAndNext };
      case RegistrationStep.BASIC_INFO:
        return { defaultValues: state.basicInfo, onNext: submitBasicInfo };
      case RegistrationStep.PROFESSIONAL:
        return { defaultValues: state.professional, onNext: submitProfessional, onBack: goBack };
      case RegistrationStep.DOCUMENTS:
        return { defaultValues: state.documents, onNext: submitDocuments, onBack: goBack };
      case RegistrationStep.VERIFICATION:
        return { defaultValues: state.preferences, onNext: submitPreferences, onBack: goBack };
