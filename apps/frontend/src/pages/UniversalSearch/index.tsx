...
interface EntityMeta { icon: LucideIcon; labelKey: string }
...
function useDebouncedValue<T>(value: T, delay = 240): T {
...
function useOfflineCache() {
  ...
  const [lastResults, setLastResults] = useState<OfflineCachePayload | null>(null);

  useEffect(() => {
    void (async () => {
      setHistory((await localforage.getItem<string[]>(HISTORY_KEY)) ?? []);
      setLastResults((await localforage.getItem<OfflineCachePayload>(LAST_KEY)) ?? null);
    })();
  }, []);
  ...
  const clearHistory = useCallback(() => {
    setHistory([]);
    localforage.removeItem(HISTORY_KEY).catch(() => {});
  }, []);

  const saveLastResults = useCallback(async (payload: OfflineCachePayload) => {
...
function useAbortablePost() {
  ...
  const run = useCallback(async (url: string, data: Record<string, unknown> = {}, config: Record<string, unknown> = {}): Promise<unknown> => {
    ref.current?.abort();
...
function useGeolocation() {
  ...
  const getLocation = useCallback((): Promise<GeoPosition | null> => {
    return new Promise((resolve) => {
      if (!('geolocation' in navigator)) { setPosition(null); resolve(null); return; }
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          const next: GeoPosition = { lat: pos.coords.latitude, lng: pos.coords.longitude, accuracy: pos.coords.accuracy };
          setPosition(next);
          resolve(next);
        },
        () => { setPosition(null); resolve(null); },
...
function useVoiceSearch(onQuery: (txt: string) => void, lang: string) {
  const w = window as Record<string, unknown>;
  const supported = typeof window !== 'undefined' && !!(w['SpeechRecognition'] ?? w['webkitSpeechRecognition']);

  const start = useCallback(() => {
    if (!supported) return;
    const SR = (w['SpeechRecognition'] ?? w['webkitSpeechRecognition']) as new() => SpeechRecognition;
    const rec = new SR();
    rec.lang = lang || 'en-US';
...
interface SmartImageProps { src?: string; alt?: string; className?: string }

function SmartImage({ src, alt, className }: SmartImageProps) {
  const [loaded, setLoaded] = useState(false);
  const [error,  setError]  = useState(false);

  const optimized = useMemo((): string | null => {
...
const ResultCard = memo(function ResultCard({ item, onClick, onHover }: ResultCardProps) {
  return (
    <motion.button
      ...
      <div className="p-4 flex gap-3 items-center">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-2 text-[11px] text-slate-500 truncate">
            ...
            {item.etaMin     != null   && <span>{item.etaMin} min ETA</span>}
...
function SearchHeader({ query, setQuery, onVoice, onQr, onClear, inputRef, voiceSupported, offline }: SearchHeaderProps) {
    ...
    <header className="sticky top-0 z-40 border-b" style={{ background: 'rgba(5,11,24,0.9)', backdropFilter: 'blur(20px)', borderColor: BORDER }}>
      <div className="mx-auto max-w-6xl px-4 py-4">
        <div className="flex items-center gap-2 rounded-2xl border px-4 py-3" style={{ background: CARD, borderColor: BORDER }} role="search">
          ...
          {query && (
            <button type="button" onClick={onClear} aria-label={t('search.clear')}>
              <X className="h-4 w-4 text-slate-500" />
            </button>
          )}
          <button type="button" onClick={onVoice} disabled={!voiceSupported} aria-label={t('search.voice')}>
            <Mic className="h-4 w-4 text-slate-500" />
          </button>
          <button type="button" onClick={onQr} aria-label={t('search.qr')}>
...
function VirtualListPanel({ items, onClick, onHover }: VListProps) {
...
function SearchErrorState({ error, retry, offline }: SearchErrorStateProps) {
  const { t } = useTranslation();
  return (
    <div className="rounded-3xl border p-6 text-center" style={{ background: CARD, borderColor: BORDER }} role="alert">
      ...
      {!offline && (
        <button type="button" onClick={retry} className="mt-4 px-5 py-2.5 rounded-full font-semibold" style={{ background: GOLD, color: '#050B18' }}>
          {t('search.retry')}
        </button>
      )}
    </div>
...
export default function UniversalSearch() {
  ...
  const [filters,     setFilters]     = useState<SearchFilters>({
    verifiedOnly: false, premiumOnly: false, openNow: false,
    priceMin: '', priceMax: '', distanceKm: '', country: '', city: '',
    category: '', ratingMin: '', language: '', availability: '',
    dateFrom: '', dateTo: '', salaryMin: '', salaryMax: '', contractType: '',
    delivery: false,
  });
  ...
  useEffect(() => {
    ...
    if (scope && scope !== 'all') next.set('type', scope); else next.delete('type');
  ...
  const searchQuery = useInfiniteQuery<SearchPageData, Error>({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      if ((pageParam as number) === 1) {
        await saveLastResults({ q: deferredQuery, scope, data: data as { items?: SearchItem[] }, ts: Date.now() });
      }
      return data as SearchPageData;
    },
    getNextPageParam: (last: SearchPageData) => last?.nextPage ?? undefined,
    staleTime: 60_000,
    gcTime: 15 * 60_000,
    retry: 3,
  ...
  const suggestionsQuery = useQuery<SuggestionsData>({
    ...
    queryFn: async () =>
      ...
      })).data as SuggestionsData,
    staleTime: 5 * 60_000,
  });

  const items = useMemo<SearchItem[]>(() => {
    const raw: SearchItem[] = (searchQuery.data?.pages ?? []).flatMap((p) => p.items ?? []);
    const source = raw.length
      ? raw
      : offline && lastResults?.q === deferredQuery
  ...
  const prefetchDetail = useCallback((item: SearchItem) => {
  ...
  const onHover = useCallback((item: SearchItem) => { prefetchDetail(item); }, [prefetchDetail]);
  ...
  const checkboxFilters: Array<[keyof SearchFilters, string]> = [
    ...
    <ErrorBoundary fallback={<div className="min-h-screen flex items-center justify-center text-slate-400 px-4">{t('search.crash')}</div>}>
      <div className="min-h-screen text-white" style={{ background: BG }}>
        <SearchHeader
          query={query}
          setQuery={setQuery}
          onVoice={() => { if (voice.supported) { voice.start(); } else { API.post('/api/search/voice', { query }).catch(() => {}); } }}
          onQr={() => navigate('/scan')}
          onClear={() => setQuery('')}
          inputRef={inputRef}
        ...
        <main className="mx-auto max-w-6xl px-4 py-6 space-y-6">
            ...
            <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
              <div className="rounded-[28px] border p-5" style={{ background: CARD, borderColor: BORDER }}>
                <h2 className="text-lg font-black flex items-center gap-2">
                  <TrendingUp className="h-5 w-5 text-amber-400" />{t('search.trending')}
                </h2>
                <div className="mt-4 flex flex-wrap gap-2">
                  {['electrician', 'cheap hotel near beach', 'driver punta cana'].map((term) => (
                    <button key={term} type="button" onClick={() => setQuery(term)} className="px-3 py-2 rounded-full text-sm border" style={{ background: '#0b1220', borderColor: BORDER }}>
                      {term}
                    </button>
                  ))}
                </div>
              ...
              <div className="rounded-[28px] border p-5" style={{ background: CARD, borderColor: BORDER }}>
                <h2 className="text-lg font-black flex items-center gap-2">
                  <History className="h-5 w-5 text-slate-400" />{t('search.recent')}
                </h2>
                <div className="mt-4 space-y-2">
                  {history.length
                    ? history.map((h) => (
                        <button key={h} type="button" onClick={() => setQuery(h)} className="w-full flex items-center justify-between rounded-2xl px-3 py-3 border text-left" style={{ borderColor: BORDER }}>
                ...
                {history.length > 0 && (
                  <button type="button" onClick={clearHistory} className="mt-4 text-xs text-slate-500">{t('search.clearHistory')}</button>
                )}
              </div>
            </section>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={() => setFiltersOpen((v) => !v)} className="ml-auto flex items-center gap-2 px-4 py-2 rounded-full border text-sm" style={{ background: CARD, borderColor: BORDER }}>
                ...
                <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-4 text-sm">
                  {checkboxFilters.map(([key, label]) => (
                    <label key={key} className="flex items-center gap-2">
                      <input type="checkbox" checked={!!filters[key]} onChange={() => setFilter(key, !filters[key] as SearchFilters[typeof key])} />
                      <span>{label}</span>
                    </label>
                  ))}
                  <input placeholder="Min price"     value={filters.priceMin}     onChange={(e) => setFilter('priceMin',     e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Max price"     value={filters.priceMax}     onChange={(e) => setFilter('priceMax',     e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Distance km"   value={filters.distanceKm}   onChange={(e) => setFilter('distanceKm',   e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Country"       value={filters.country}      onChange={(e) => setFilter('country',      e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="City"          value={filters.city}         onChange={(e) => setFilter('city',         e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Language"      value={filters.language}     onChange={(e) => setFilter('language',     e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Category"      value={filters.category}     onChange={(e) => setFilter('category',     e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Rating min"    value={filters.ratingMin}    onChange={(e) => setFilter('ratingMin',    e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Date from"     value={filters.dateFrom}     onChange={(e) => setFilter('dateFrom',     e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Date to"       value={filters.dateTo}       onChange={(e) => setFilter('dateTo',       e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Salary min"    value={filters.salaryMin}    onChange={(e) => setFilter('salaryMin',    e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Salary max"    value={filters.salaryMax}    onChange={(e) => setFilter('salaryMax',    e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                  <input placeholder="Contract type" value={filters.contractType} onChange={(e) => setFilter('contractType', e.target.value)} className="rounded-xl bg-white/5 border border-white/10 px-3 py-2" />
                </div>
                <div className="mt-4">
                  <button type="button" onClick={() => void getLocation()} className="inline-flex items-center gap-2 px-4 py-2 rounded-full border" style={{ borderColor: BORDER }}>
          ...
          {searchQuery.isError && !offline && (
            <SearchErrorState error={searchQuery.error} retry={() => void searchQuery.refetch()} offline={offline} />
          )}
          ...
          {offline && (
            <div className="rounded-2xl border px-4 py-3 text-sm flex items-center gap-2" style={{ background: 'rgba(250,204,21,0.08)', borderColor: GOLD }}>
              <WifiOff className="h-4 w-4" /> {t('search.offlineBanner')}
            </div>
          )}
            ...
            <section className="rounded-[24px] border p-4" style={{ background: CARD, borderColor: BORDER }}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {suggestionsQuery.data!.items!.map((s) => (
                  <button key={s} type="button" onClick={() => setQuery(s)} className="px-3 py-2 rounded-full bg-white/5 border border-white/10 text-sm">{s}</button>
                ))}
              </div>
            </section>
          )}
              ...
              {scope === 'all'
                ? Object.entries(grouped).map(([type, arr]) => (
                    <div key={type} className="space-y-3">
                      ...
                      <VirtualListPanel items={arr} onClick={onClick} onHover={onHover} />
                    </div>
                  ))
                : <VirtualListPanel items={grouped[scope] ?? []} onClick={onClick} onHover={onHover} />
              }
