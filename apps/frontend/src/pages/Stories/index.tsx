...
interface Story {
  id: number; author: string; role: string; emoji: string; color: string;
  duration: number; viewed: boolean; caption: string;
}
...
export default function Stories() {
  ...
  const nextStory = () => {
    if (active !== null && active < MOCK_STORIES.length - 1) { setActive(active + 1); setProgress(0); }
    else closeStory();
  };

  const prevStory = () => {
    if (active !== null && active > 0) { setActive(active - 1); setProgress(0); }
  };
      ...
      <div className="px-4">
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-col gap-3">
            {MOCK_STORIES.filter(s => !s.viewed).map(s => (
