...
interface StatusEntry { color: string; bg: string; label: string; }
...
const FALLBACK_STATUS: StatusEntry = STATUS_CFG['pending']!;
...
function CreateServiceModal({ onClose, onSave }: CreateServiceModalProps) {
  const { t } = useTranslation();
  const [form, setForm] = useState<ServiceForm>({ name:'', category:'', price:'', unit:'pa travay', description:'', active:true });
      ...
      <div className="relative w-full bg-[#0d1526] rounded-t-3xl p-5 pb-10 z-10 max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder={t('provider.serviceName', { defaultValue:'Non sèvis la…' })}
          ...
          <input value={form.category} onChange={e => set('category', e.target.value)}
            placeholder={t('provider.category', { defaultValue:'Kategori…' })}
            className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm placeholder-slate-500 focus:outline-none focus:border-amber-500/50" />
...
export default function ServiceProviderDashboard() {
  ...
  const [services,   setServices]   = useState<Service[]>(() => {
    try { return JSON.parse(localStorage.getItem('jf_sp_services') ?? 'null') as Service[] | null ?? MOCK_SERVICES; } catch { return MOCK_SERVICES; }
  });
  ...
  useEffect(() => {
    ...
    if (!userId) return;
    API.get<unknown>('/bookings', { params: { userId } })
      .then(r => {
        const res = r?.data as { data?: unknown } | unknown[] | null;
        const d = (res as { data?: unknown })?.data ?? res;
        if (Array.isArray(d) && d.length) {
          setBookings((d as Record<string, unknown>[]).map(b => ({
            _id:     (b['_id']  ?? b['id']           ?? '') as string,
            client:  (b['clientName'] ?? b['client'] ?? 'Kliyan') as string,
            service: (b['serviceName'] ?? b['service'] ?? 'Sèvis') as string,
            date:    (b['date'] ?? b['scheduledDate']  ?? '') as string,
            time:    (b['time'] ?? b['scheduledTime']  ?? '') as string,
  ...
  const stats      = userRecord?.['stats'] as { totalJobs?: number; rating?: number; reviews?: number } | undefined;
  ...
  const profileMeta = userRecord?.['profileMetadata'] as { profilePhoto?: string } | undefined;
  const avatarSrc   = profileMeta?.profilePhoto
    ?? `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent((userRecord?.['name'] as string) ?? 'sp')}`;
      ...
      <div className="p-4">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-700 via-blue-700 to-cyan-800 p-5 shadow-2xl">
          ...
          <div className="flex items-center gap-3 relative z-10">
            <img src={avatarSrc} alt={userRecord?.['name'] as string}
              className="w-14 h-14 rounded-2xl object-cover border-2 border-white/20 shadow-xl" />
            <div className="min-w-0">
              <p className="font-black text-white text-base truncate">{(userRecord?.['name'] as string) ?? 'Provider'}</p>
              <p className="text-blue-200 text-xs truncate">{(userRecord?.['profession'] as string) ?? 'Prestatè Sèvis'}</p>
          ...
          <>
            ...
            <div>
              ...
              {bookings.slice(0, 3).map(b => {
                const cfg = STATUS_CFG[b.status] ?? FALLBACK_STATUS;
                return (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button type="button" onClick={() => navigate('/create-post')}
                className="text-xs text-amber-400 font-bold">+ {t('provider.addMedia', { defaultValue:'Ajoute' })}</button>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {((userRecord?.['portfolio'] as string[] | undefined) ?? []).length === 0 ? (
              ...
              ) : ((userRecord?.['portfolio'] as string[]) ?? []).map((img, i) => (
