...
// Done simulation ki parèt sou imaj MVP la (image_44.png)
const MOCK_RESULTS = [
  {
    id: 1,
    name: "Mason",
...
const SearchResults = () => {
  ...
  const [filterKm, setFilterKm] = useState("2 km");
    ...
    <div className="min-h-screen w-full bg-[#0B1528] text-white flex flex-col font-sans select-none pb-28">
      ...
      <div className="px-5 pt-6 pb-3 max-w-md w-full mx-auto flex items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          />
          <button className="absolute inset-y-0 right-4 flex items-center text-slate-400">
      ...
      {/* 🎛️ FILTÈ KOUTIM (Pills ki nan image_44.png lan) */}
      ...
      <div className="px-5 mt-3 flex-1 flex flex-col gap-3 max-w-md mx-auto w-full overflow-y-auto">
        {MOCK_RESULTS.map((worker) => (
          ...
          >
            ...
            {/* Pri ak Status Dot bò kote li */}
            <div className="text-right flex flex-col items-end gap-1.5">
              <span className="text-xs font-black text-amber-400 tracking-wide">{worker.rate}</span>
              <div className="flex items-center gap-1.5">
                <span className={`w-2 h-2 rounded-full ${worker.dotColor}`}></span>
                <span className={`text-[9px] font-extrabold uppercase tracking-wider ${worker.textColor}`}>{worker.statusLabel}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* 🧭 NAVIGASYON ANBA (Floating Bottom Bar) */}
