...
const TOKENS = {
  ...
  goodBg: 'bg-emerald-500',
  warn: 'text-orange-300',
  warnBg: 'bg-orange-400',
  danger: 'text-red-300',
  dangerBg: 'bg-red-500',
  blueBg: 'bg-sky-500',
  blue: 'text-sky-300',
...
const HERO_MEDIA = {
  ...
  overlay:
    'radial-gradient(circle at 20% 20%, rgba(56,189,248,.18), transparent 26%), radial-gradient(circle at 80% 12%, rgba(245,158,11,.16), transparent 25%), radial-gradient(circle at 60% 72%, rgba(34,197,94,.10), transparent 24%), linear-gradient(180deg, rgba(2,6,23,.20), rgba(2,6,23,.78))',
};
...
function MetricBar({ label, value, total, tone = 'bg-amber-400' }) {
...
function HeroCinematic({ company, branches, businessType, onOpenActions }) {
      ...
      <div className="relative grid gap-5 p-5 md:p-6 lg:grid-cols-[1.15fr_.85fr]">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[11px] font-bold text-slate-200">
              {bt.icon} {bt.label}
            </span>
            <span className="rounded-full border border-sky-400/20 bg-sky-400/10 px-3 py-1 text-[11px] font-bold text-sky-200">
              120 FPS motion ready
            </span>
          </div>

          <div className="max-w-3xl">
            <h1 className="text-3xl font-black leading-none tracking-tight md:text-5xl">
              {company?.name || 'Enterprise Global OS'}
            </h1>
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="group rounded-full bg-white px-5 py-3 text-sm font-black text-slate-950 transition hover:scale-[1.02]"
            >
              <span className="inline-flex items-center gap-2">
                Open Command Center <span className="transition group-hover:translate-x-1">→</span>
              </span>
            </button>
            <button
              onClick={onOpenActions}
              className="rounded-full border border-white/15 bg-white/10 px-5 py-3 text-sm font-bold text-white backdrop-blur-xl"
            >
              Launch AI Assistant
            </button>
          </div>
            ...
            <div className="relative">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
                  ...
                  <div className="mt-1 text-2xl font-black text-emerald-300">${liveRevenue.toFixed(0)}K</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
                  <div className="text-xs text-slate-400">Active Users</div>
                  <div className="mt-1 text-2xl font-black text-sky-300">{Math.round(liveUsers)}</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
                  <div className="text-xs text-slate-400">Alerts</div>
                  <div className="mt-1 text-2xl font-black text-amber-300">{Math.round(liveAlerts)}</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
                  <div className="text-xs text-slate-400">Server</div>
                  <div className="mt-1 text-2xl font-black text-white">99.98%</div>
                </div>
              </div>
            </div>
          </div>
...
function WorldMapPanel({ branches }) {
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={x.code} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/35 p-4">
              ...
              <div className="text-sm font-black text-amber-300">{Math.round((x.workers / total) * 100)}%</div>
            </div>
          ))}
        </div>
      </div>
...
function RecruitmentPanel({ onHire }) {
  return (
    <SectionFrame
      title="Recruitment"
...
function WorldOperationsPanel({ branches }) {
  return (
...
function EmployeesPanel({ branches }) {
  const total = branches.reduce((s, b) => s + b.employees, 0);
  const online = branches.reduce((s, b) => s + b.online, 0);
  return (
    <SectionFrame title="Employees" subtitle="Live workforce">
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <StatCard icon="⚡" value={fmt(branches.reduce((s, b) => s + b.working, 0))} label="Working" color="text-amber-300" />
        <StatCard icon="🏖" value={fmt(branches.reduce((s, b) => s + b.vacation, 0))} label="Vacation" color="text-sky-300" />
      </div>
      <div className="mt-4 space-y-2">
...
function OverviewGrid({ branches, onHire }) {
  return (
    <div className="grid gap-4 xl:grid-cols-[1.4fr_.9fr]">
      <div className="space-y-4">
        <HeroCinematic
          company={{ name: 'JOBFAST Enterprise', verified: true }}
          branches={branches}
          businessType="enterprise"
          onOpenActions={onHire}
        />
  ...
  return (
    <motion.section layout className="space-y-4">
      <div className={cn('flex items-center gap-3 p-4', TOKENS.panelStrong, TOKENS.radius.xl)}>
        ...
        <div className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-[10px] font-bold text-emerald-300">
          Aktif
        </div>
      </div>

      <div className="flex gap-2 overflow-x-auto pb-1">
        {['overview', 'employees', 'jobs', 'reservations', 'customers', 'inventory', 'finance', 'settings'].map(bt => (
      ...
      <AnimatePresence mode="wait">
        {tab === 'overview' && (
          ...
          >
            <StatCard icon="👥" value={empStats.total} label="Total anplwaye" />
            <StatCard icon="🟢" value={empStats.online} label="Anliy kounye a" color="text-emerald-300" />
            <StatCard icon="⚡" value={empStats.working} label="Ap travay" color="text-amber-300" />
            <StatCard icon="🏖" value={empStats.vacation} label="Vakans" color="text-sky-300" />
          ...
          >
            {MOCK_EMPLOYEES_ALL.filter(e => e.branch === branch.name).map(e => (
              <div key={e.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                ...
                <div className={cn('text-xs font-bold', e.online ? 'text-emerald-300' : 'text-slate-400')}>
                  {e.status}
                </div>
              </div>
            ))}
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {editing ? (
                ...
                <button onClick={() => setEditing(true)} className="text-xs font-bold text-amber-300">
                  Modifye
                </button>
              )}
            </div>
        ...
        {['jobs', 'reservations', 'customers', 'inventory', 'finance'].includes(tab) && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="rounded-3xl border border-white/10 bg-white/5 p-8 text-center text-slate-300"
          >
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full rounded-2xl border border-dashed border-amber-400/30 bg-amber-400/5 py-3 text-sm font-bold text-amber-300"
      >
        + Kreye Branch
      </button>
          ...
          >
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <input
                ...
                placeholder="Vil"
                className="rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 text-sm outline-none"
              />
              <select
                value={newBranch.country}
                onChange={e => setNewBranch(p => ({ ...p, country: e.target.value }))}
                className="rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 text-sm outline-none"
              >
            ...
            <input
              ...
              placeholder="Branch Manager (opsyonèl)"
              className="w-full rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 text-sm outline-none"
            />
            <div className="flex gap-2">
              <button onClick={createBranch} className="flex-1 rounded-2xl bg-amber-400 px-4 py-3 text-sm font-black text-slate-950">
                Kreye
              </button>
              <button onClick={() => setShowCreate(false)} className="rounded-2xl border border-white/10 px-4 py-3 text-sm text-slate-300">
                Anile
              </button>
            </div>
          </motion.div>
      ...
      <div className="space-y-3">
        {filtered.map(branch => (
          <button
            ...
            className="w-full rounded-[1.6rem] border border-white/10 bg-white/5 p-4 text-left backdrop-blur-xl transition hover:bg-white/10"
          >
            <div className="flex items-center gap-3">
              <div className="grid h-12 w-12 place-items-center rounded-2xl border border-white/10 bg-sky-400/10 text-2xl">
                {branch.flag}
              </div>
              <div className="min-w-0 flex-1">
...
function EmployeesTab({ branches }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <Pill label="Tout" active={statusFilter === 'all'} onClick={() => setStatusFilter('all')} />
        <Pill label="Working" active={statusFilter === 'working'} onClick={() => setStatusFilter('working')} />
        <Pill label="Online" active={statusFilter === 'online'} onClick={() => setStatusFilter('online')} />
        <Pill label="Vacation" active={statusFilter === 'vacation'} onClick={() => setStatusFilter('vacation')} />
        <Pill label="Offline" active={statusFilter === 'offline'} onClick={() => setStatusFilter('offline')} />
      </div>
...
function ActionSheet({ open, onClose, onHire, onNavigate }) {
  return (
    <AnimatePresence>
      {open ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={onHire} className="rounded-2xl bg-amber-400 px-4 py-4 text-left font-black text-slate-950">
                Hire workers
              </button>
              <button onClick={() => onNavigate('regions')} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-left font-bold text-white">
                Manage branches
              </button>
              <button onClick={() => onNavigate('employees')} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-left font-bold text-white">
                Employees
              </button>
              <button onClick={() => onNavigate('finance')} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 text-left font-bold text-white">
                Finance
              </button>
            </div>
            <button onClick={onClose} className="mt-3 w-full rounded-2xl border border-white/10 px-4 py-3 text-sm text-slate-300">
              Close
            </button>
          </motion.div>
        </motion.div>
...
function HireWorkersModal({ open, onClose, onPublish }) {
...
function GlobalSections({ activeSection, branches, setBranches, countryFilter, setCountryFilter, onHire }) {
  return (
    <div className="grid gap-4 xl:grid-cols-[1.4fr_.9fr]">
      <div className="space-y-4">
        <div data-section="executive"><OverviewGrid branches={branches} onHire={onHire} /></div>
        ...
        <div data-section="operations"><WorldOperationsPanel branches={branches} /></div>
        <div data-section="regions"><BranchesTab branches={branches} setBranches={setBranches} countryFilter={countryFilter} setCountryFilter={setCountryFilter} /></div>
        <div data-section="departments"><DepartmentsPanel /></div>
        <div data-section="employees"><EmployeesTab branches={branches} /></div>
        <div data-section="customers"><CustomersPanel /></div>
        <div data-section="reservations"><SectionFrame title="Reservations" subtitle="Bookings / schedule"><ReservationsTab /></SectionFrame></div>
        <div data-section="inventory"><SectionFrame title="Inventory" subtitle="Stock / supply"><InventoryTab /></SectionFrame></div>
        <div data-section="finance"><FinanceTab /></div>
        <div data-section="payroll"><PayrollPanel /></div>
        <div data-section="recruitment"><RecruitmentPanel onHire={onHire} /></div>
        <div data-section="marketplace"><MarketplacePanel /></div>
        <div data-section="security"><SecurityPanel /></div>
        <div data-section="ai"><AIExecutivePanel /></div>
...
function BranchListTab({ branches, setBranches, countryFilter, setCountryFilter }) {
  const [selectedBranch, setSelectedBranch] = useState(null);

  if (selectedBranch) {
    return <BranchDetail branch={selectedBranch} onBack={() => setSelectedBranch(null)} onSave={updated => setBranches(prev => prev.map(b => (b.id === updated.id ? updated : b)))} />;
  }

  return <BranchesTab branches={branches} setBranches={setBranches} countryFilter={countryFilter} setCountryFilter={setCountryFilter} />;
}

export default function EnterpriseDashboard({ company }) {
