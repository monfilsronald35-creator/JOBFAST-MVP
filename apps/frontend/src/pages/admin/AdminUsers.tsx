...
interface UserRecord {
  ...
  role?: string; category?: string; accountStatus?: string;
  verified?: boolean; createdAt?: string;
  location?: { city?: string };
}

type UserStatusKey = 'active' | 'suspended' | 'banned' | 'verified';

const STATUS_STYLE: Record<UserStatusKey, { cls: string; label: string }> = {
...
const statusOf = (u: UserRecord): UserStatusKey =>
...
type ActionType = 'verify' | 'suspend' | 'ban';

interface UserModalProps {
  user: UserRecord;
  onClose: () => void;
  onAction: (type: ActionType, userId: string, reason: string) => Promise<void>;
}

function UserModal({ user, onClose, onAction }: UserModalProps) {
  const [loading, setLoading] = useState(false);
  const [reason,  setReason]  = useState('');

  const act = async (type: ActionType) => {
    setLoading(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
      <div className="w-full max-w-md bg-[#0d1526] border border-slate-700 rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-black text-lg">{user.name ?? 'Unknown'}</h3>
...
export default function AdminUsers() {
  ...
  const [modal,   setModal]   = useState<UserRecord | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await API.get<unknown>('/admin/users', { params });
      const d = res.data as { data?: { items?: UserRecord[]; total?: number } | UserRecord[] } | null;
      const inner = (d as { data?: { items?: UserRecord[]; total?: number } })?.data;
      if (inner && !Array.isArray(inner)) {
        setUsers(inner.items ?? []);
        setTotal(inner.total ?? 0);
      } else {
        const arr = (d as { data?: UserRecord[] })?.data ?? (Array.isArray(d) ? d : []) as UserRecord[];
        setUsers(arr);
        setTotal(arr.length);
      }
    } catch {
  ...
  const handleAction = async (type: ActionType, userId: string, reason: string) => {
    try {
