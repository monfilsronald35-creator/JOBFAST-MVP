...
import {
  ...
  Download, RefreshCw, ToggleLeft, ToggleRight, Users, Activity,
} from 'lucide-react';
...
const API_BASE = '/api/v1/admin/governance';

// ── Data interfaces ───────────────────────────────────────────

interface AuditEntry { id: string; type: string; action?: string; actorId?: string; targetId?: string; createdAt?: string; }
interface AuditData  { entries?: AuditEntry[]; total?: number; hasMore?: boolean; }

interface ModerationItem {
  id: string; itemType: string; status: string;
  userId?: string; userName?: string; userRole?: string;
  comment?: string; description?: string; category?: string;
  verificationType?: string; flagScore?: number;
}
interface ModerationData { items?: ModerationItem[]; total?: number; }

interface FraudUser { userId: string; name?: string; email?: string; role?: string; fraudScore?: number; spamScore?: number; accountStatus?: string; fraudSignals?: string[]; }
interface FraudSummary { totalUsers?: number; totalFraudFlags?: number; totalSpamFlags?: number; highRiskCount?: number; }
interface FraudScanResult { scanned?: number; flagged?: number; }
interface FraudData { flagged?: FraudUser[]; summary?: FraudSummary; }

interface Dispute { id: string; status: string; category?: string; description?: string; targetUserName?: string; targetUserRole?: string; adminNotes?: string; }
interface DisputeData { disputes?: Dispute[]; }

interface FeatureFlag { id: string; name: string; description?: string; enabled: boolean; emergencyDisabled?: boolean; rolloutPct?: number; isBeta?: boolean; roles?: string[]; }
interface FlagsData { flags?: FeatureFlag[]; }

interface LaborRules  { minAge?: number; minWagePerDay?: number; minWageCurrency?: string; maxHoursPerWeek?: number; overtimeMultiplier?: number; }
interface PaymentRules { methods?: string[]; defaultMethod?: string; refundWindowDays?: number; }
interface TaxRules    { vatRate?: number; servicesTaxRate?: number; }
interface Country     { code: string; name: string; active: boolean; currency?: string; timezone?: string; laborRules: LaborRules; paymentRules: PaymentRules; taxRules: TaxRules; }
interface CountriesData { countries?: Country[]; }

type PermMatrix = Record<string, Record<string, boolean>>;
interface LocalMatrix { matrix?: PermMatrix; }
interface PermissionsData { matrix?: PermMatrix; }

interface ConsentLog  { id: string; userId?: string; type: string; granted: boolean; ip?: string; createdAt?: string; }
interface ConsentData { logs?: ConsentLog[]; total?: number; }

interface HealthUsers       { total?: number; active?: number; suspended?: number; banned?: number; }
interface HealthMemory      { heapUsedMb?: number; heapTotalMb?: number; rssMb?: number; }
interface HealthFeatureFlags { total?: number; enabled?: number; }
interface HealthData { users?: HealthUsers; uptime?: number; memory?: HealthMemory; featureFlags?: HealthFeatureFlags; consentLogs?: number; timestamp?: string; }

// ── Shared hooks ─────────────────────────────────────────────

function useGovAPI<T>(path: string, deps: React.DependencyList = []) {
  const { token } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (j.success) setData(j.data ?? null);
  ...
  // path and token are the real deps; `deps` arg intentionally controls re-runs from caller side
...
async function govPost(path: string, body: unknown, token: string | null | undefined, method = 'POST') {
  const r = await fetch(`${API_BASE}${path}`, {
    method,
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token ?? ''}` },
    body: JSON.stringify(body),
  });
  return r.json() as Promise<{ success?: boolean; data?: unknown }>;
}

// ── Shared UI ────────────────────────────────────────────────

interface BadgeProps { color: string; bg: string; children: React.ReactNode; }
function Badge({ color, bg, children }: BadgeProps) {
  return <span style={{ color, background: bg }} className="px-2 py-0.5 rounded-full text-xs font-medium">{children}</span>;
}
...
interface StatProps { label: string; value: number | string; icon: LucideIcon; color?: string; }
function Stat({ label, value, icon: Icon, color = '#3b82f6' }: StatProps) {
  return (
...
interface AuditFilters { type: string; search: string; page: number; }

function AuditLogs() {
  const { token } = useAuth();
  const [filters, setFilters] = useState<AuditFilters>({ type: '', search: '', page: 1 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${API_BASE}/audit-logs?${params.toString()}`, { headers: { Authorization: `Bearer ${token ?? ''}` } });
      const j = await r.json() as { success?: boolean; data?: AuditData };
      if (j.success) setData(j.data ?? null);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    const r = await fetch(`${API_BASE}/audit-logs/export`, { headers: { Authorization: `Bearer ${token ?? ''}` } });
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              placeholder="Rechèch..." value={filters.search}
        ...
        <div className="flex items-center justify-between text-sm text-gray-500">
          ...
          <div className="flex gap-2">
            <button disabled={filters.page <= 1} onClick={() => setFilters(f => ({ ...f, page: f.page - 1 }))}
              className="px-3 py-1 border rounded disabled:opacity-40 dark:border-gray-600 dark:text-white">Prev</button>
            <span className="px-2 py-1">P{filters.page}</span>
            <button disabled={!data.hasMore} onClick={() => setFilters(f => ({ ...f, page: f.page + 1 }))}
              className="px-3 py-1 border rounded disabled:opacity-40 dark:border-gray-600 dark:text-white">Next</button>
          </div>
        </div>
      )}
    </div>
...
function ModerationCenter() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetch(`${API_BASE}/moderation?type=${typeFilter}`, { headers: { Authorization: `Bearer ${token ?? ''}` } });
      const j = await r.json() as { success?: boolean; data?: ModerationData };
      if (j.success) setData(j.data ?? null);
    } finally {
      setLoading(false);
    }
  ...
  const handleAction = async (item: ModerationItem, action: string) => {
    prompt(`Rezon pou "${action}" (opsyonèl):`);
    setActing(item.id);
    const actualId = item.id.replace(/^(rev_|cmp_|usr_)/, '');
    try {
      await govPost(`/moderation/${item.itemType}/${actualId}`, { action }, token);
      await load();
    } finally {
      setActing(null);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Badge color={COMPLAINT_STATUS_CONFIG[item.status]?.color ?? '#6b7280'} bg={(COMPLAINT_STATUS_CONFIG[item.status]?.color ?? '#6b7280') + '22'}>
                      {item.itemType}
                    </Badge>
                    ...
                    {item.flagScore != null && <Badge color="#ef4444" bg="#fee2e2">Score: {item.flagScore}</Badge>}
                  </div>
                  <p className="text-sm font-medium text-gray-900 dark:text-white truncate">
                    {item.userName ?? item.userId}
                    {item.userRole && <span className="text-gray-400 ml-1">({item.userRole})</span>}
                  </p>
                  {item.comment     && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{item.comment}</p>}
                  {item.description && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{item.description}</p>}
                  {item.category    && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Kategori: {item.category}</p>}
                  {item.verificationType && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Tip: {item.verificationType}</p>}
                </div>
                <div className="flex gap-1.5 flex-shrink-0">
                  {actions.map(a => (
                    <button key={a.id} onClick={() => handleAction(item, a.id)} disabled={acting === item.id}
        ...
        {!loading && !data?.items?.length && <p className="text-center text-gray-400 py-8">Pa gen atik pou modere</p>}
      </div>
      {data && <p className="text-sm text-gray-500 text-right">{data.total} atik total</p>}
    </div>
  );
}
...
function FraudDetection() {
  const { token } = useAuth();
  const { data, loading, reload } = useGovAPI<FraudData>('/fraud', []);
  const [scanning,   setScanning]   = useState(false);
  const [scanResult, setScanResult] = useState<FraudScanResult | null>(null);

  const runScan = async () => {
    setScanning(true);
    const j = await govPost('/fraud/scan', {}, token) as { success?: boolean; data?: FraudScanResult };
    if (j.success) setScanResult(j.data ?? null);
    setScanning(false);
    reload();
  };
    ...
    <div className="space-y-4">
      ...
      {data?.summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Stat label="Total Itilizatè"  value={data.summary.totalUsers ?? 0}      icon={Users}         color="#3b82f6" />
          <Stat label="Siyalman Fwòd"    value={data.summary.totalFraudFlags ?? 0} icon={AlertTriangle} color="#f97316" />
          <Stat label="Siyalman Spam"    value={data.summary.totalSpamFlags ?? 0}  icon={Flag}          color="#f59e0b" />
          <Stat label="Risk Wo"          value={data.summary.highRiskCount ?? 0}   icon={Shield}        color="#ef4444" />
        </div>
      )}
      ...
      <div className="space-y-2">
          ...
          <Card key={u.userId}>
            <div className="flex items-center justify-between">
              <div>
                ...
                {u.fraudSignals && u.fraudSignals.length > 0 && (
                  <div className="flex gap-1 mt-1 flex-wrap">
                    {u.fraudSignals.map(s => <Badge key={s} color="#ef4444" bg="#fee2e2">{s}</Badge>)}
                  </div>
                )}
              </div>
              <div className="text-right">
                ...
                <Badge color={u.accountStatus === 'active' ? '#22c55e' : '#ef4444'} bg={u.accountStatus === 'active' ? '#dcfce7' : '#fee2e2'}>
                  {u.accountStatus}
                </Badge>
              </div>
            </div>
        ...
        {!loading && !data?.flagged?.length && <p className="text-center text-gray-400 py-8">Pa gen itilizatè risk wo</p>}
      </div>
    </div>
  );
}
...
function DisputeResolution() {
  const { token } = useAuth();
  const { data, loading, reload } = useGovAPI<DisputeData>('/disputes', []);
  const [acting, setActing] = useState<string | null>(null);

  const update = async (id: string, status: string, resolution: string | null) => {
    setActing(id);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
          ...
          <Card key={d.id}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <Badge color={COMPLAINT_STATUS_CONFIG[d.status]?.color ?? '#6b7280'} bg={COMPLAINT_STATUS_CONFIG[d.status]?.bg ?? '#f3f4f6'}>
              ...
              <div className="flex flex-col gap-1.5">
                <button onClick={() => update(d.id, 'resolved', 'in_favor')} disabled={acting === d.id}
                  className="px-3 py-1 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50">
                  Rezoud
                </button>
                <button onClick={() => update(d.id, 'rejected', null)} disabled={acting === d.id}
        ...
        {!loading && !data?.disputes?.length && <p className="text-center text-gray-400 py-8">Pa gen dispute aktif</p>}
      </div>
    </div>
  );
}
...
function FeatureFlags() {
  const { token } = useAuth();
  const { data, loading, reload } = useGovAPI<FlagsData>('/feature-flags', []);
  ...
  const emergencyOff = async (flag: FeatureFlag) => {
    await govPost(`/feature-flags/${flag.id}`, { emergencyDisabled: !flag.emergencyDisabled }, token);
    reload();
  };

  const updateRollout = async (flag: FeatureFlag, pct: string) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  {editing === flag.id ? (
                    <div className="flex items-center gap-2">
                      <input type="range" min={0} max={100} defaultValue={flag.rolloutPct}
                        className="w-24"
                        onMouseUp={e => updateRollout(flag, (e.target as HTMLInputElement).value)} />
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => emergencyOff(flag)} title="Koupe ijans"
        ...
        {!loading && !data?.flags?.length && <p className="text-center text-gray-400 py-8">Pa gen fonksyon</p>}
      </div>
    </div>
  );
}
...
function CountryConfiguration() {
  const { token } = useAuth();
  const { data, loading, reload } = useGovAPI<CountriesData>('/countries', []);
    ...
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {(data?.countries ?? []).map(c => (
          <button key={c.code} onClick={() => { setSelected(c.code); setEditing({}); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            ...
            <div className="space-y-2 text-sm">
              {([['Kòd', country.code], ['Lajan', country.currency ?? '—'], ['Timezone', country.timezone ?? '—']] as [string, string][]).map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  <span className="text-gray-500">{k}:</span>
                  <span className="font-mono text-gray-900 dark:text-white text-xs">{v}</span>
                </div>
              ))}
              <div className="flex justify-between items-center">
                <span className="text-gray-500">Aktif:</span>
                <button onClick={() => setEditing(e => ({ ...e, active: !(editing.active !== undefined ? editing.active : country.active) }))} className="focus:outline-none">
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              {([
                ['Laj min',       `${country.laborRules.minAge ?? '—'} an`],
                ['Salè min/jou',  `${country.laborRules.minWagePerDay ?? '—'} ${country.laborRules.minWageCurrency ?? ''}`],
                ['Max hr/semèn',  `${country.laborRules.maxHoursPerWeek ?? '—'}h`],
                ['Ekstra-tan',    `×${country.laborRules.overtimeMultiplier ?? '—'}`],
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              {([
                ['Metòd',        (country.paymentRules.methods ?? []).join(', ')],
                ['Defòlt',       country.paymentRules.defaultMethod ?? '—'],
                ['Rembourseman', `${country.paymentRules.refundWindowDays ?? '—'} jou`],
              ] as [string, string][]).map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  <span className="text-gray-500">{k}:</span>
                  <span className="text-gray-900 dark:text-white text-xs">{v}</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              {([
                ['TVA',        `${((country.taxRules.vatRate ?? 0) * 100).toFixed(0)}%`],
                ['Taks Sèvis', `${((country.taxRules.servicesTaxRate ?? 0) * 100).toFixed(0)}%`],
              ] as [string, string][]).map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  <span className="text-gray-500">{k}:</span>
                  <span className="text-gray-900 dark:text-white">{v}</span>
      ...
      {!selected && !loading && <p className="text-center text-gray-400 py-8">Chwazi yon peyi pou wè konfigirasyon</p>}
    </div>
  );
}
...
function PermissionMatrix() {
  const { token } = useAuth();
  const { data, loading, reload } = useGovAPI<PermissionsData>('/permissions', []);
  const [saving, setSaving] = useState<string | null>(null);
  const [local,  setLocal]  = useState<LocalMatrix>({});

  const matrix: PermMatrix = local.matrix ?? data?.matrix ?? {};
        ...
        <table className="text-xs w-full">
          <thead>
            <tr className="border-b dark:border-gray-700">
              ...
              <th className="py-2 px-2 text-gray-500 dark:text-gray-400" />
            </tr>
          </thead>
          <tbody className="divide-y dark:divide-gray-700">
                ...
                {permissions.map(perm => {
                  ...
                  return (
                    <td key={perm} className="py-2 px-1 text-center">
                      <button onClick={() => toggle(role, perm)} className="focus:outline-none"
                        title={`${PERMISSION_LABELS[perm] ?? perm}: ${val ? 'Aktif' : 'Inaktif'}`}>
                        {val
...
function ConsentTracking() {
  ...
  const search = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/consent${params}`, { headers: { Authorization: `Bearer ${token ?? ''}` } });
      const j = await r.json() as { success?: boolean; data?: ConsentData };
      if (j.success) setData(j.data ?? null);
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Search size={14} className="absolute left-2 top-2.5 text-gray-400" />
          <input placeholder="Filtè pa userId..." value={userId} onChange={e => setUserId(e.target.value)}
          ...
          <tbody className="divide-y dark:divide-gray-700">
            {(data?.logs ?? []).map(l => (
              <tr key={l.id} className="text-gray-700 dark:text-gray-300">
                <td className="py-2 pr-4 text-xs font-mono">{l.userId ? l.userId.slice(0, 12) + '…' : '—'}</td>
...
function SystemHealth() {
  const { data, loading, reload } = useGovAPI<HealthData>('/health', []);
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              {([
                ...
                ['RAM Itilize', `${data.memory?.heapUsedMb ?? 0} MB`],
                ['RAM Total',   `${data.memory?.heapTotalMb ?? 0} MB`],
                ['RSS',         `${data.memory?.rssMb ?? 0} MB`],
              ] as [string, string][]).map(([k, v]) => (
                <div key={k} className="flex justify-between">
                  <span className="text-gray-500">{k}:</span>
                  <span className="text-gray-900 dark:text-white">{v}</span>
