...
function Toast({ msg, type = 'success', onClose }: ToastProps) {
  useEffect(() => {
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    API.get<{ data?: Record<string, unknown> } | Record<string, unknown>>('/admin/settings')
      .then(res => {
        const d = (res.data as { data?: Record<string, unknown> })?.data ?? res.data as Record<string, unknown>;
        setMaintenance((d?.maintenance as boolean) ?? false);
        setRegOpen((d?.registrationOpen as boolean) ?? true);
        setGpsEnabled((d?.gpsEnabled as boolean) ?? true);
        setMaxSearch((d?.maxUsersPerSearch as number) ?? 50);
      })
      .catch(() => {})
  ...
  const sendGlobalNotif = async () => {
    ...
    try {
      ...
      const pushRes = await API.post<{ data?: { sent?: number } }>('/push/send-all', { title: notifTitle || 'JOBFAST', body: notifMsg, url: '/notifications' });
  ...
  interface Tool { icon: string; label: string; fn: () => void; cls: string; }
  const TOOLS: Tool[] = [
    ...
    { icon:'📋', label:'Audit Logs',     fn: () => { /* TODO */ },                                     cls:'text-purple-400 border-purple-500/20 bg-purple-500/10 hover:bg-purple-500/20' },
    ...
    <div className="p-6 max-w-3xl space-y-6">
      ...
      <div className="bg-[#0d1526] border border-slate-800/60 rounded-2xl p-5">
        ...
        {loading
          ...
          : <>
              <Toggle checked={maintenance} onChange={toggleMaintenance} label="Maintenance Mode" description="When ON, the app shows a maintenance page to all users" />
              <Toggle checked={regOpen}     onChange={setRegOpen}      label="Open Registration"  description="Allow new users to create accounts" />
              <Toggle checked={gpsEnabled}  onChange={setGpsEnabled}   label="GPS / Location"     description="Enable GPS-based features and job matching" />
              <div className="py-3 border-b border-slate-800/40">
                ...
                <input type="range" min={10} max={200} step={10} value={maxSearch} onChange={e => setMaxSearch(+e.target.value)} className="w-full accent-amber-400" />
                <div className="flex justify-between text-[10px] text-slate-600 mt-1"><span>10</span><span>200</span></div>
              </div>
              <div className="pt-3">
      ...
      <div className="bg-[#0d1526] border border-slate-800/60 rounded-2xl p-5">
        ...
        <div className="space-y-3">
          <label className="block">
            ...
            <input value={notifTitle} onChange={e => setNotifTitle(e.target.value)} placeholder="e.g. Platform maintenance tonight"
          ...
          <label className="block">
            ...
            <textarea value={notifMsg} onChange={e => setNotifMsg(e.target.value)} rows={3} placeholder="Write your message to all users…"
