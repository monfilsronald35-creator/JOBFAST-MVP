...
interface MarketCategory { id: string; icon: string; label: string }
interface SellerStory    { id: string; label: string; icon: string; mine?: boolean }
...
type MarketAPI = {
  getListings?:    (p: { category?: string }) => Promise<{ data?: unknown }>;
  toggleFavorite?: (id: string | number) => Promise<unknown>;
  createListing?:  (data: Record<string, unknown>) => Promise<{ data?: unknown }>;
};

const api = marketplaceAPI as unknown as MarketAPI;
...
const SELLER_STORIES: SellerStory[] = [
...
function ListingCard({ item, favorites, onFavorite, onOpen, isLast }: ListingCardProps) {
  ...
  return (
    <>
      ...
      >
        <div
          className="w-[64px] h-[64px] rounded-[16px] flex items-center justify-center text-[30px] shrink-0 overflow-hidden border"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-[11px] text-slate-400 font-medium">Verified {item.sellerType ?? 'Seller'}</span>
          </div>
        ...
        <div className="flex flex-col items-end gap-3 shrink-0">
          <button
            type="button"
            aria-label={isFav ? 'Retire favori' : 'Ajoute favori'}
...
function DetailModal({ item, onClose, onContact }: DetailModalProps) {
  if (!item) return null;
  return (
    <div className="fixed inset-0 z-50 flex flex-col overflow-y-auto" style={{ background: BG }}>
      ...
      <div className="h-52 flex items-center justify-center text-8xl border-b" style={{ background: CARD, borderColor: BORDER }}>
        {item.imageUrl
          ? <img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover" />
      ...
      <div className="p-5 space-y-4 pb-28">
        <div className="flex items-start justify-between">
          ...
          <span className="text-xs text-slate-400 px-3 py-1.5 rounded-full font-medium" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
            Négociable
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={m.label} className="p-3 rounded-[16px]" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
              ...
              <p className="text-sm font-bold text-white mt-0.5">{String(m.value)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div
        className="fixed bottom-0 left-0 right-0 p-4 flex gap-3 border-t"
...
function CreateModal({ onClose, onSubmit }: CreateModalProps) {
  const [form, setForm] = useState<CreateForm>({ title: '', category: 'products', price: '', currency: 'USD', city: '', description: '' });
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: BG }}>
      <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderColor: BORDER }}>
        <button type="button" onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-[14px] text-slate-300 font-bold text-lg" style={style}>✕</button>
        <h2 className="font-black text-white flex-1 text-[15px]">Nouvo Lis</h2>
      </div>

      <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-5 space-y-4 pb-28">
        <div
          className="h-36 rounded-[20px] flex flex-col items-center justify-center gap-2 cursor-pointer transition-all"
        ...
        <div>
          ...
          <input type="text" value={form.title} onChange={e => set('title', e.target.value)} placeholder="Ex: iPhone 16 Pro 256GB" required className={inputCls} style={style} />
        </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            ...
            <input type="number" value={form.price} onChange={e => set('price', e.target.value)} placeholder="0" min="0" required className={inputCls} style={style} />
          </div>
          <div className="w-24">
        ...
        <div>
          ...
          <input type="text" value={form.city} onChange={e => set('city', e.target.value)} placeholder="Punta Cana" className={inputCls} style={style} />
        </div>

        <div>
          <label className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Deskripsyon</label>
          <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={4} placeholder="Dekri pwodui ou a…" className={`${inputCls} resize-none`} style={style} />
        </div>
      </form>
      ...
      <div className="p-4 border-t" style={{ background: BG, borderColor: BORDER }}>
        <button
          ...
          disabled={busy || !form.title || !form.price}
...
export default function MarketPage() {
  ...
  useEffect(() => {
    setLoading(true);
    api.getListings?.({ category: activeCategory !== 'all' ? activeCategory : undefined })
      .then(res => {
        const raw = res?.data as { data?: Listing[] } | Listing[] | undefined;
        const data = Array.isArray(raw) ? raw : (raw as { data?: Listing[] } | undefined)?.data;
  ...
  const handleFavorite = useCallback((id: string | number) => {
    setFavorites(prev => {
  ...
  const handleContact = useCallback((item: Listing) => {
    navigate(`/chat?sellerId=${item.sellerId ?? String(item._id)}`);
  }, [navigate]);

  const handleCreate = useCallback(async (data: CreateForm) => {
    try {
      const res     = await api.createListing?.({ ...data, sellerId: u?.['_id'] });
      const raw     = res?.data as { data?: Listing } | Listing | undefined;
      const created = (Array.isArray(raw) ? undefined : (raw as { data?: Listing } | undefined)?.data ?? raw) as Listing | undefined;
      setListings(p => [
        created ?? { _id: Date.now(), ...data, price: Number(data.price), seller: u?.['name'] as string | undefined, postedAt: 'jis kounye a', image: '📦', verified: false },
        ...p,
      ]);
    } catch {
      setListings(p => [
        { _id: Date.now(), ...data, price: Number(data.price), seller: u?.['name'] as string | undefined, postedAt: 'jis kounye a', image: '📦', verified: false },
        ...p,
      ]);
    }
    setShowCreate(false);
    ...
    <div className="flex flex-col min-h-screen pb-24 text-white" style={{ background: BG }}>
      ...
      <div className="sticky top-14 z-30 border-b" style={{ background: `${BG}FC`, backdropFilter: 'blur(20px)', borderColor: BORDER }}>
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-[16px] transition-all" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
      ...
      <div className="flex gap-3.5 overflow-x-auto px-4 pt-4 pb-2" style={{ scrollbarWidth: 'none' }}>
        {SELLER_STORIES.map(s => (
          ...
          >
            <div
              className="w-[56px] h-[56px] rounded-full flex items-center justify-center text-[22px] border-2 transition-all"
              style={s.mine
      ...
      <div className="flex-1 px-4 pb-4">
        ...
        <div className="rounded-[20px] overflow-hidden shadow-2xl shadow-black/40" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
          {loading
            ? [1, 2, 3].map((n, i, a) => (
                <div key={n}>
      ...
      {detailItem && (
        <DetailModal item={detailItem} onClose={() => setDetailItem(null)} onContact={handleContact} />
      )}
      {showCreate && (
