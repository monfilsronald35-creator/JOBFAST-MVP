...
function MapController({ position, following }: MapControllerProps) {
  const map = useMap();
...
export default function MapNavigationScreen() {
  ...
  const initCenter: [number, number] = userPos ?? [18.5432, -72.3395];
        ...
        {routeCoords && routeCoords.length > 0 && (
          <>
            <Polyline
              ...
              pathOptions={{ color: "#000", weight: 9, opacity: 0.18 }}
            />
            <Polyline
              positions={routeCoords}
      ...
      <div className="absolute top-0 left-0 right-0 z-[500] px-3 pt-3 space-y-2 pointer-events-none">
        ...
        {results && (results as SearchResult[]).length > 0 && (
          <div className="pointer-events-auto bg-[#0f172a]/98 border border-slate-700 rounded-2xl overflow-hidden shadow-2xl">
            {(results as SearchResult[]).map((r, index) => (
              <button
          ...
          <div className="pointer-events-auto bg-[#0f172a]/97 border border-amber-500/40 rounded-2xl px-4 py-3 shadow-2xl flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-amber-400 mt-0.5">
                {metersToLabel((activeStep as NavStep).distance ?? 0)} · Etap{" "}
                {curStep + 1}/{(steps as NavStep[]).length}
              </p>
            </div>
            <button
              type="button"
              onClick={() => setVoiceOn((v: boolean) => !v)}
              className={
      ...
      <div className="absolute bottom-0 left-0 right-0 z-[500] px-3 pb-3 space-y-2">
        <div className="flex justify-between items-end">
          ...
          <div className="flex items-center gap-1 bg-[#0f172a]/90 border border-slate-700 rounded-full px-2 py-1 text-[10px] text-slate-300 shadow-xl">
            {(["driving", "walking", "bicycle", "truck"] as const).map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => setMode(m)}
                className={
                  ...
                  (mode === m ? "bg-amber-500 text-slate-950" : "text-slate-300")
                }
              >
                {m === "driving" ? "🚗" : m === "walking" ? "🚶" : m === "bicycle" ? "🚲" : "🚚"}
              </button>
            ))}
          </div>

          {navigating && steps && (steps as NavStep[]).length > 0 && (
            <button
              type="button"
            ...
            >
              ...
              Tout etap ({(steps as NavStep[]).length})
            </button>
          )}
        </div>
          ...
          <div className="bg-[#0f172a]/97 border border-slate-700/80 rounded-2xl px-4 py-3 flex items-center gap-3 shadow-2xl">
            <div className="flex-1 flex items-center gap-4 min-w-0">
              <div className="flex flex-col gap-0.5 shrink-0">
                ...
                {eta && (
                  <span className="text-[10px] text-slate-400">ETA {eta}</span>
                )}
              </div>
              ...
              {destName && (
                <p className="text-[10px] text-slate-400 truncate">{destName}</p>
              )}
            </div>
                ...
                <div className="flex-1 text-left min-w-0">
                  <p
                    className={
                      ...
                      (i === curStep ? "text-amber-400" : "text-slate-200")
                    }
                  >
                    {stepInstruction(s)}
                  </p>
